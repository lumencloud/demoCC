sap.ui.define([
    ...
    return Controller.extend("bix.card.accountDetailTableAiReport.Main", {
        ...
        onUiChange: async function (oEvent) {
            ...
            // 테이블 병합
            await Module.setTableMerge(this.byId("accountDetailTable"), "oAccountTableModel", 1);
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            let sAccountPath = `/get_cstco_by_biz_account(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',account_cd='${sOrgId}')`
            const oBinding = oPlModel.bindContext(sAccountPath);
            let aResults = await oBinding.requestObject();

            
            //선택된 행의 type 값 가져오기 
            if(oAiData.aiType ==="마진"){
                aResults = aResults.value[0].margin;
            }else if(oAiData.aiType ==="마진률"){
                aResults = aResults.value[0].margin_rate;
            }else if(oAiData.aiType ==="매출"){
                aResults = aResults.value[0].sale;
            }
            
            //formatter를 위해서 type 값 추가 
            aResults.forEach(item=>{
                item.type=oAiData.aiType;
            })

            this.getView().setModel(new JSONModel(aResults), "oAccountTableModel");
            // 테이블 로우 셋팅
        ...
        _setBusy: async function (bType) {
            this.byId("accountDetailTable").setBusy(bType)
        },

        _setVisibleRowCount: function (aResults) {


            let aModelArray = this.getView().getModel("oAccountTableModel").getData()
        ...
        onFormatPerformance: function (iValue1, iValue2, sType, sTooltip) {
            console.log("iValue1  : ",iValue1)
            console.log("iValue2  : ",iValue2)
            console.log("sType  :   ",sType)
            console.log("sTooltip  : ",sTooltip)
