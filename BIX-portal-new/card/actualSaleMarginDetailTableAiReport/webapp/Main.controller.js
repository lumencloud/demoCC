    ...
    return Controller.extend("bix.card.actualSaleMarginDetailTableAiReport.Main", {
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            await Promise.all([
                ...
                //모델바인딩
                this.getView().setModel(new JSONModel(aFilteredResults[0]), "oOrgTableModel")
                this.getView().setModel(new JSONModel(aFilteredResults[1]), "oAccountTableModel")
                this.getView().setModel(new JSONModel(aFilteredResults[2]), "oTernalTableModel")
                this.getView().setModel(new JSONModel(aFilteredResults[3]), "oOverTableModel")
                this.getView().setModel(new JSONModel(aFilteredResults[4]), "oOwnTableModel")
