  ...
  function (BaseController, EventBus, ODataModel, JSONModel, NumberFormat) {
    ...
    return BaseController.extend("bix.card.saleCrov.card", {
      ...
      _dataSetting: async function () {
        ...
        let aResult = await oModel.bindContext(`/get_actual_sale_crov_pl(year='${iYear}',month='${sMonth}',org_id='${sOrgId}')`).requestObject();

        let oResult = aResult.value.find((result) => result.org_nm === "합계" && result.type === "매출")

        let oChangeData = {
          "new": oResult.actual_curr_ym_value,
          "crov": oResult.actual_last_ym_value,
          "newRate": oResult.actual_curr_ym_value / (oResult.actual_curr_ym_value + oResult.actual_last_ym_value),
          "crovRate": oResult.actual_last_ym_value / (oResult.actual_curr_ym_value + oResult.actual_last_ym_value)
        }
