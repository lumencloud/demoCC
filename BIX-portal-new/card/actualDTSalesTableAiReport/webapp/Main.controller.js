sap.ui.define([
    ...
    "../../main/util/Module",
], function (Controller, JSONModel, ODataModel, NumberFormat, EventBus, Modules, InteractionUtils, AgentService, AIPopupManager, MessageBox, HashChanger, Module) {
    "use strict";
     ...
     */
    return Controller.extend("bix.card.actualDTSalesTableAiReport.Main", {
        /**
        ...
        _setBusy: function (bFlag) {
            let aTableLists = ["actualDTSalesBox1", "actualDTSalesBox2", "actualDTSalesBox3"]
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            let sAccountPath = `/get_cstco_by_biz_account_dt(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',account_cd='${sOrgId}')`
            let sTaskPath = `/get_actual_dt_task_oi(year='${iYear}',month='${sMonth}',org_id='${sOrgId}')`

            console.log("sAccountPath,   ", sAccountPath)
            await Promise.all([
                ...
                this.getView().setModel(new JSONModel(aResults[1].value[0].sale), "oAccountTableModel")
                this.getView().setModel(new JSONModel(aResults[2].value), "oTaskTableModel")
                // 테이블 로우 셋팅
                this._setVisibleRowCount(aResults);

                console.log("aresult  : ", new JSONModel(aResults[1].value[0].sale))
        ...
        _setVisibleRowCount: function (aResults) {
            //테이블 리스트
            let aTableLists = ["actualDTSalesTable1", "actualDTSalesTable2", "actualDTSalesTable3"]
        ...
        onCellContextmenu: function (oEvent) {
            ...
            if (oSelect.getSelectedKey() === "task" || oSelect.getSelectedKey() === "account") {
                return
            }
        ...
        _startAnalysis: function (oEvent, oParams, sAnalysisId) {
            ...
            const table_id = oEvent.getSource().getId();
            let func_nm;
            if (table_id === "actualDTSalesTable1") {
                func_nm = "get_actual_dt_org_oi"
            }
            else if (table_id === "actualDTSalesTable2") {
                func_nm = "get_actual_dt_account_oi"
            }
        ...
        _setTableMerge: async function () {
            ...
            let aModelList = ["oOrgTableModel", "oAccountTableModel", "oTaskTableModel"]

            for (let i = 0; i < aTableList.length; i++) {
                Module.setTableMerge(this.byId(aTableList[i]), aModelList[i], 1)
            }
        },
