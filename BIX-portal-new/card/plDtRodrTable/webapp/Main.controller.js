sap.ui.define([
	...
	return Controller.extend("bix.card.plDtRodrTable.Main", {
		onInit: function () {
			this._bindTable();
		},
		...
		_bindTable: async function () {
			let oTable = this.byId("plDtRodrTable");
			if (!oTable) {
				return;
			};
			...
			let oColumn0 = new sap.ui.table.Column({
				template: new sap.m.Text({ text: "{plDtRodrTableModel>dtTask}", emptyIndicatorMode: "On"}),
				hAlign: "Center",
				width: "12rem"
			});
            ...
            const oPlBindingContext = oPlModel.bindContext(`/get_rodr_dt_y`);
			...
			aYear.forEach(data=>{
				let oColumn1 = new sap.ui.table.Column({
					template: new sap.m.Text({
						text: {
							parts:[
								{path: `plDtRodrTableModel>${'column'+data}`},
								{value: ``}
							],							
						...
						tooltip: {
							parts: [
								{ path: `plDtRodrTableModel>${'column'+data}`, targetType: 'any' },
								{ value: 'tooltip' }
							], formatter: this.onFormatPerformance.bind(this)
						},
			...
			this.getView().setModel(new JSONModel(aResults.value), "plDtRodrTableModel");
			this.getView().setModel(new JSONModel({cardInfo:"plDtRodrTable"}),"selectModel");
        ...
        onExcelDownload: async function () {
            ...
            // 데이터 반환
            const oPlModel = this.getOwnerComponent().getModel("pl_api");
            const oPlBindingContext = oPlModel.bindContext(`/get_rodr_dt_y_excel`);
            ...
            link.href = url;
            link.download = `Rodr DT Raw Data_${iYear}-${iMonth}.xlsx`;
            link.click();
