module.exports = (srv) => {
    srv.on('get_forecast_opp_br', async (req) => {
        ...
        // rsp 조회용 정보
        const rsp_column = ['ccorg_cd', 'biz_opp_no', 'prj_tp_nm', 'sum(ifnull(received_order_amt,0)) as received_order_amt', 'sum(ifnull(sales_amt,0)) as sales_amt', 'sum(ifnull(sales_amt,0)*(1-(ifnull(margin_rate,0)/100))) as labor_cost'];
