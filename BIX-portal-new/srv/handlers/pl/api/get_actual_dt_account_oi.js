...
module.exports = (srv) => {
    srv.on('get_actual_dt_account_oi', async (req) => {
        ...
        const pl_where_conditions = { 'year': { in: [year, last_year] }, 'src_type': { 'not in':['WO','D']},'dgtr_task_cd': {'!=':null, and : {'dgtr_task_cd': {'!=':''}}}};
        ...
        let o_total = { "display_order": 0, "account_cd": 'total', "account_nm": '합계'};
        ...
        account_data.forEach((data,i)=>{
            ...
            a_pl_data.forEach((o_pl_data)=>{
                if(o_pl_data.account_cd === data.biz_tp_account_cd){
                    ...
                    o_temp["actual_curr_ym_rate"] = (flat_target?.[`_${o_pl_data.account_cd}_${year}_dt_sale_target`] ?? 0) === 0 ? 0 : (o_pl_data?.['curr_sale'] ?? 0) / ((flat_target?.[`_${o_pl_data.account_cd}_${year}_dt_sale_target`] ?? 0) * 100000000) *100
                    o_temp["actual_last_ym_rate"] = (flat_target?.[`_${o_pl_data.account_cd}_${last_year}_dt_sale_target`] ?? 0) === 0 ? 0 : (o_pl_data?.['last_sale'] ?? 0) / ((flat_target?.[`_${o_pl_data.account_cd}_${last_year}_dt_sale_target`] ?? 0) * 100000000)*100
                };
                
                if(i===0){
                    // o_total['target_curr_y_value'] = (o_total['target_curr_y_value'] || 0) + ;
                    ...
                    iLastYearTotalTarget += (flat_target?.[`_${o_pl_data.account_cd}_${last_year}_dt_sale_target`] ?? 0);
                };
            });
            oResult.push(o_temp);
        });
        ...
        o_total['actual_last_ym_rate'] = iLastYearTotalTarget === 0 ? 0 : (o_total?.['actual_last_ym_value'] ?? 0) / (iLastYearTotalTarget*100000000)*100;
