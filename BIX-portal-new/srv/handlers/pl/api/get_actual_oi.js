...
module.exports = (srv) => {
    srv.on('get_actual_oi', async (req) => {
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            a_sale_total_column.push(`sum(ifnull(sale_m${i}_amt,0))`)
        ...
        let target_column = org_level.includes('div') || org_level.includes('hdqt') ? [...target_col_list, 'max(ifnull(target_br_mm_amt,0)) as target_br_mm_amt'] : target_col_list
        const target_groupBy = ['target_year']

        const target_rohc_column = ['target_year', 'ifnull(target_rohc,0) as target_rohc'];
        const target_rohc_where = { 'target_year': { in: [year] }, total: false, org_ccorg_cd: org_ccorg_cd  };
        ...
        const rsp_column = ['year',
            ...
            s_rsp_total_amt_ym, s_rsp_total_amt_ym_total
        ];
        const rsp_where = { 'year': { in: [year, last_year] }, [org_level]: orgInfo.org_ccorg_cd, is_delivery: true };
        const rsp_groupBy = ['year'];

        // DB 쿼리 실행 (병렬)
        const [target_data, target_rohc_data, pl_data, non_mm, sga_data, dt_pl_data, rsp_data] = await Promise.all([
            // PL 실적, 목표 조회
            SELECT.from(target_view).columns(target_column).where(target_where).groupBy(...target_groupBy),
            SELECT.from(target_view).columns(target_rohc_column).where(target_rohc_where),
        ...
        const sale_data =
        {
            ...
            "actual_curr_ym_rate": (curr_target?.dt_target_sale_amt ?? 0) !== 0 ? (dt_pl_curr_y_row?.["sale_amount_sum"] ?? 0) / (curr_target?.dt_target_sale_amt * 100000000) : 0,
            // "actual_last_ym_rate": (last_target?.dt_target_sale_amt ?? 0) !== 0 ? (dt_pl_last_y_row?.["sale_amount_sum"] ?? 0) / (last_target.dt_target_sale_amt * 100000000) : 0
        ...
        const non_mm_data =
        {
            ...
            "actual_curr_ym_rate": (curr_target?.non_mm_target_sale_amt ?? 0) === 0 ? 0 : (curr_non_mm?.sale_amount_sum ?? 0) / 100000000 / curr_target.non_mm_target_sale_amt,
            "actual_last_ym_rate": (last_target?.non_mm_target_sale_amt ?? 0) === 0 ? 0 : (last_non_mm?.sale_amount_sum ?? 0) / 100000000 / last_target.non_mm_target_sale_amt
        };
        ...
        const br_data =
        {
            "display_order": 4,
            "type": "BR",
            "target_curr_y_value": (curr_target?.target_br_mm_amt ?? 0) / 100,
            "actual_curr_ym_value": rsp_curr_y_row?.mm_value ?? 0,
            "actual_last_ym_value": rsp_last_y_row?.mm_value ?? 0,
            "actual_curr_ym_rate": (curr_target?.target_br_mm_amt ?? 0) === 0 ? 0 : (rsp_curr_y_row?.mm_month_sum_value ?? 0)/curr_target.target_br_mm_amt,
        ...
        const rohc_data =
        {
            "display_order": 5,
            "type": "RoHC",
            "target_curr_y_value": target_rohc_data?.[0]?.target_rohc ?? 0,
            "actual_curr_ym_value": (rsp_curr_y_row?.['total_amt_sum'] ?? 0) !== 0 ? ((pl_curr_y_row?.["margin_amount_sum"] ?? 0) - (sga_curr_y_row?.["sga_amount_sum"] ?? 0)) / rsp_curr_y_row['total_amt_sum'] : 0,
            "actual_last_ym_value": (rsp_last_y_row?.['total_amt_sum'] ?? 0) !== 0 ? ((pl_last_y_row?.["margin_amount_sum"] ?? 0) - (sga_last_y_row?.["sga_amount_sum"] ?? 0)) / rsp_last_y_row['total_amt_sum'] : 0,
            "actual_curr_ym_rate": (target_rohc_data?.[0]?.target_rohc ?? 0) === 0 || (rsp_curr_y_row?.['total_amt_sum'] ?? 0) === 0 ? 0 : (((pl_curr_y_row?.["margin_amount_sum"] ?? 0) - (sga_curr_y_row?.["sga_amount_sum"] ?? 0)) / rsp_curr_y_row['total_amt_sum']) / (target_rohc_data?.[0]?.target_rohc),
            "actual_last_ym_rate": (rsp_last_y_row?.total_amt_year_sum ?? 0) === 0 || (rsp_last_y_row?.['total_amt_sum'] ?? 0) === 0 ? 0 : (((pl_last_y_row?.["margin_amount_sum"] ?? 0) - (sga_last_y_row?.["sga_amount_sum"] ?? 0)) / rsp_last_y_row['total_amt_sum']) / (((pl_last_y_row?.margin_total_amount_sum ?? 0) - (sga_last_y_row?.sga_total_amount_sum ?? 0)) / (rsp_last_y_row?.total_amt_year_sum)), // 작년 1~마감월(조직별 월별 공헌이익 / 총인건비) / 작년1~12 (조직별 월별 공헌이익 / 총인건비)
        };
