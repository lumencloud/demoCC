...
// return org_query
        ...
        // DB 쿼리 실행 (병렬)
        let ㅌ, target_data, target_ent_data;
        ...
        let total_data = [
            {
                ...
                "org_id": "total",
                "org_nm": org_col_nm === 'hdqt_id' || org_col_nm === 'team_id'? org_col_nm_name : '합계',
                "type": '매출',
                "target_curr_y_value": org_col_nm === 'lv1_id' ? Number(oCurrTargetEnt?.target_sale_amt ?? 0) : 0,
            ...
            },
            {
                ...
                "org_nm": org_col_nm === 'hdqt_id' || org_col_nm === 'team_id'? org_col_nm_name : '합계',
                "type": '마진',
                "target_curr_y_value": org_col_nm === 'lv1_id' ? (Number((oCurrTargetEnt?.target_sale_amt ?? 0)) * Number((oCurrTargetEnt?.target_margin_rate ?? 0)) / 100) : 0,
                "target_last_y_value" : 0,
                "actual_curr_ym_value": 0,
                "actual_last_ym_value": 0,
                "actual_curr_ym_rate": 0,
            ...
            },
            {
                ...
                "org_nm": org_col_nm === 'hdqt_id' || org_col_nm === 'team_id'? org_col_nm_name : '합계',
                "type": '마진률',
                "target_curr_y_value": org_col_nm === 'lv1_id' ? Number((oCurrTargetEnt?.target_margin_rate ?? 0)) : 0,
                "target_last_y_value" : 0,
                "actual_curr_ym_value": 0,
                "actual_last_ym_value": 0,
                "actual_curr_ym_rate": 0,
            ...
            let sale_data =
                {
                    ...
                    "actual_curr_ym_rate": Number((flat_target?.[`_${data.id}_${year}_target_sale_amt`] ?? 0)) === 0 ? 0 : (flat_pl?.[`_${data.id}_${year}_sale_amount_sum`] ?? 0) / (Number((flat_target?.[`_${data.id}_${year}_target_sale_amt`] ?? 0))*100000000),
            ...
            let margin_data =
                {
                    ...
                    "actual_curr_ym_rate": margin_target_curr === 0 ? 0 : (flat_pl?.[`_${data.id}_${year}_margin_amount_sum`] ?? 0) / (margin_target_curr * 100000000),
            ...
            let margin_rate_data =
                {
                    ...
                    "org_nm": data.name,
                    "type": "마진률",
                    "target_curr_y_value": Number(flat_target?.[`_${data.id}_${year}_target_margin_rate`] ?? 0)/100,
       ...
       if(org_col_nm === 'lv2_id' || org_col_nm === 'lv3_id'){
            target_data.forEach(data => {
                if(data.target_year === year){
                    total_data[0].target_curr_y_value += Number(data?.target_sale_amt ?? 0);
                    total_data[1].target_curr_y_value += Number(data?.target_sale_amt ?? 0) * Number(data?.target_margin_rate ?? 0) / 100;
                }
            });
            total_data[2].target_curr_y_value = (total_data?.[0].target_curr_y_value ?? 0) === 0 ? 0 : (total_data?.[1].target_curr_y_value ?? 0) / (total_data?.[0].target_curr_y_value ?? 0) ;
        }else if(org_col_nm === 'div_id' || org_col_nm === 'hdqt_id'|| org_col_nm === 'team_id' ){
            total_data[0].target_curr_y_value = Number((flat_target?.[`_${org_id}_${year}_target_sale_amt`] ?? 0));
            total_data[1].target_curr_y_value = Number((flat_target?.[`_${org_id}_${year}_target_sale_amt`] ?? 0)) * Number((flat_target?.[`_${org_id}_${year}_target_margin_rate`] ?? 0)) / 100;
            total_data[2].target_curr_y_value = Number((flat_target?.[`_${org_id}_${year}_target_margin_rate`] ?? 0));
        };
        
        total_data.forEach((data,i)=>{
            if(i === 2){
                data.actual_curr_ym_rate = 0
                data.actual_last_ym_rate = 0
            }else{
                data.actual_curr_ym_rate = (data?.target_curr_y_value ?? 0) === 0 ? 0 : (data?.actual_curr_ym_value ?? 0) / ((data?.target_curr_y_value ?? 0) *100000000);
                data.actual_last_ym_rate = (data?.target_last_y_value ?? 0) === 0 ? 0 : (data?.actual_last_ym_value ?? 0) / (data?.target_last_y_value ?? 0);
            };
        });
