module.exports = (srv) => {
    srv.on('get_actual_rsp_org_detail_excel', async (req) => {
        //pl 디테일 테이블 엑셀 파일 다운로드 rsp 부분.(pl, pl target, sga, rsp각각 handler 작성)
        ...
        const [rsp_query] = await Promise.all([
            // rsp 데이터
            SELECT.from(rsp_view).where(rsp_where)
        ]);

        oResult.push(...rsp_query);
        return oResult;
    });
}