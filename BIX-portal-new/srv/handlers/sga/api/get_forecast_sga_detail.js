...
module.exports = (srv) => {
    srv.on('get_forecast_sga_detail', async (req) => {
        ...
        for(let i=1; i<=12; i++){
            if(i<=i_month){
                a_exp_secured.push(`sum(ifnull(exp_m${i}_amt,0))`);
                a_labor_secured.push(`sum(ifnull(labor_m${i}_amt,0))`);
                a_iv_secured.push(`sum(ifnull(iv_m${i}_amt,0))`);
            }else{
                a_exp_not_secured.push(`sum(ifnull(exp_m${i}_amt,0))`);
                a_iv_not_secured.push(`sum(ifnull(iv_m${i}_amt,0))`);
            }
        }
        ...
        const rsp_col_list = ['year', 'month_amt', 'sum(ifnull(opp_amt,0)) as opp_amt', 'sum(ifnull(opp_amt_sum,0)) as opp_amt_sum'];
