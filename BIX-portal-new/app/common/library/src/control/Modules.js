sap.ui.define([
    ...
    **/
    ...
    Modules.setIconTab = function (oEvent) {
        ...
        const oInfo = oData.filter(item => item.category === aHash[0] && item.code === aHash[1] && item.route === null || item.route === sRoute)
            ...
            oData.forEach(parent => {
                if (parent.Child && parent.Child.length > 0) {
                    parent.Child.forEach(child => {
                        if (child.category === aHash[0] && child.code === aHash[1] && child.route === null || child.route === sRoute) {
                            result.push(child);
                        }
                    })
                }
    ...
    /**
     * @param {String} sType 매출, 마진, 마진률,....percent(진척률)
    ...
    Modules.valueFormat = function (sType, iValue1, iValue2, tooltipType) {
        ...
        if (sType === "마진률" || sType === "영업이익률" || sType === "BR" || sType === "percent") {
            if (!iNewValue) {
                return '0%'
            }
