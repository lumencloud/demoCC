sap.ui.define([
     ...
     */
    return Controller.extend("bix.master.orgtarget.controller.AccountTarget", {
        /**
         * @type {Array} 
         */
        _aAddTargetData: [],
        ...
        onMyRoutePatternMatched: function () {
            ...
            let oAddTargetTable = this.byId("accountTargetTable");
        ...
        _setExcelTable: function(oTable){
            if(oTable.getColumns().length>0){
                ...
                let oInput = new Input({
                    ...
                    value:'{excelUploadModel>Account}',
                ...
                aChangedData.forEach((oData) => {
                    this._aPropertyList.forEach((oProperty) => {
                        ...
                        if ((oProperty["value"] !=="A02" && bChangeData) || bChangeTotalYn) {
                            ...
                            let sBindingPath = `/AnnualTarget(year='${iYear}',target_type='biz_tp_account_cd',target_type_cd='${oData.account_cd}',target_cd='${oProperty["value"]}')`
                            ...
                            if (isExist) {
                                ...
                                let oCreateObject = {
                                    year: iYear.toString(),
                                    target_type:"biz_tp_account_cd",
                                    target_type_cd: oData.account_cd,
        ...
        _setData: async function (oEvent, sEventId, oData) {
            if(!sEventId || oData.type === "Account별"){
                this.getView().setBusy(true);

                // 데이터 호출
                let sPlUrl = "/get_org_target(type='account')";
        ...
        onExcelTemplateDownload: function () {
            ...
            // 열 구성
            let aColumns = ["Account"]
            let aColumnSetting = [{key : "Account",width : 25}];
            this._aPropertyList.forEach(oProperty => {
                if(oProperty.value !== 'D01'){
            ...
            aBindingModel.forEach(function (oContext) {
                let oExcelRow = [oContext.account_nm, ...Array(15).fill(null)];
            ...
            workbook.xlsx.writeBuffer().then((buffer) => {
                ...
                link.download = `Account별 목표실적 템플릿.xlsx`;
                link.click();
            });
        },
                        ...
                        if (aExcelData.length > 0) {
                            if (!this._excelUploadDialog) {
                                ...
                                await this.loadFragment({
                                    id: "accountExcelUpload",
                                    name: `${sComponentName}.view.fragment.AccountExcelUpload`,
                                    controller: this,
                            ...
                            aExcelData = aExcelData.filter(oExcelData => {
                                let aDataVal = Object.entries(oExcelData).filter(([key]) => key !== "Account");
            ...
            if (sFlag === "Department") { // 부서
                if (sValue) {
                    ...
                    if (!!oData) {
                        ...
                        oSource.setValueStateText("입력한 Account는 존재하지 않습니다.");
                    }
                } else {
                    oSource.setValueState(coreLib.ValueState.Error);
        ...
        _setHighlight:function(){
            let oTable = this.byId('accountTargetTable');
        ...
        /**
         * AccountExcelUpload 필드 LiveChange 이벤트
         * @param {Event} oEvent 
         * @param {String} sFlag Department, Number
         */
        ...
        _validateField: function (sValue, sFlag) {
            if (sFlag === "Department") {
                ...
                return aTargetData.find(function (oDepartmentData) {
                    return oDepartmentData.account_nm.replaceAll(" ", "").toLowerCase()
            ...
            if (sFlag === "Save") {
                ...
                if (!!bInvalid) {
                    ...
                    aAddTargetTableData.forEach(oData =>{
                        oData.new_A02_data = oData.new_A01_data && oData.new_A03_data ? this.onMarginRate(oData.new_A01_data, oData.new_A03_data) : 0;
                    })
