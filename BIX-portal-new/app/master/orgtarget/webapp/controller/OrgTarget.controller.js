sap.ui.define([
    ...
    return Controller.extend("bix.master.orgtarget.controller.OrgTarget", {
        ...
        _setTable: function(oTable){
            this._aPropertyList.forEach(oProperty =>{
                if(oProperty.value !== 'D01'){
                    ...
                    if(['A02','A05','A07'].includes(oProperty.value)){
                        oInput = new Input({
                            width:'100%',
                            textAlign:'End',
                            fieldGroupIds:'Input',
        ...
        onExcelUploadDialogButton: function (oEvent, sFlag) {
            if (sFlag === "Save") {
                ...
                if (!!bInvalid) {
                    ...
                    this._setMarginRate(aAddTargetTableData)
        ...
        _setMarginRate:function(oData){
            oData.forEach(data => {
                let iSales = data.new_A01_data
                let iMarginRate = data.new_A02_data
                let iMargin = data.new_A03_data
                if (iSales && iMargin && !iMarginRate) {
                    data.new_A02_data = this.onMarginRate(iSales, iMargin)
                }else if (iSales && !iMargin && iMarginRate) {
                    data.new_A03_data = this.onMargin(iSales, iMarginRate)
                }else if (!iSales && iMargin && iMarginRate) {
                    data.new_A01_data = this.onSales(iMargin, iMarginRate)
                }else if (iSales && iMargin && iMarginRate) {
                    data.new_A02_data = this.onMarginRate(iSales, iMargin)
                }
                if(data.children.length>0){
                    this._setMarginRate(data.children)
                }
            })
        },
