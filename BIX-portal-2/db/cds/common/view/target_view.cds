...
view annual_target_base_view as
    select
        ...
        max(case
                when
                    target.target_cd = 'A07'
                then
                    target.target_val
            end) as br_cost_target,
        max(case
                when
                    target.target_cd = 'B01'
                then
        ...
        max(case
                ...
                then
                    target.target_val
            end) as total_labor_target,
        max(case
                when
                    target.target_cd = 'C05'
                then
                    target.target_val
            end) as labor_target,
        max(case
                when
                    target.target_cd = 'C06'
                then
                    target.target_val
            end) as iv_target,
        max(case
                when
                    target.target_cd = 'C07'
                then
                    target.target_val
            end) as cost_target,
        ifnull(target.is_total_calc,false)  as is_total
...
view annual_target_temp_view as select from 
    (
        select
            org.org_id,
            org.org_ccorg_cd,
