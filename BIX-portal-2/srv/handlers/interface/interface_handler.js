...
module.exports = class InterfaceService extends cds.ApplicationService {
  ...
  init() {
    ...
    // WBS Project
    this.on('execute_wbs_prj_batch', async (req) => {
      const { ver } = req.data;
      ...
      const empty_prj = await SELECT.distinct.from(pl_if_wideview).columns('prj_no')
        .where({ prj_no: { 'not in': SELECT.distinct.from(common_if_project).columns('prj_no').where({ 'ver': ver }) }, 'ver': ver });
      ...
      const RCV_func = async (RCV_PARAM) => {
        ...
        if (RCV_PARAM.is_yn) {
          ...
          if (RCV_PARAM.api_parameter === 'YEAR_MONTH') {
            oData = { ...oData, 'YEAR_MONTH': `${year}${month}` } // 임시 `${year}${month}` 로 대체
          } else if (RCV_PARAM.api_parameter === 'cal_dt__c') {
            oData = { ...oData, 'cal_dt__c': '250425001' }// 임시
          }
