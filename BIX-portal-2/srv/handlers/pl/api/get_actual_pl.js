module.exports = (srv) => {
    srv.on('get_actual_pl', async (req) => {
        ...
        const profit_data =
        {
            ...
            "actual_curr_ym_rate": (curr_target?.profit_target_amt ?? 0) === 0 ? 0 : ((contribution_data["actual_curr_ym_value"]) - (sga_exp_curr_y_row?.["sga_amount_sum"] ?? 0))/(curr_target.profit_target_amt * 100000000) * 100,
            "actual_last_ym_rate": (last_target?.profit_target_amt ?? 0) === 0 ? 0 : ((contribution_data["actual_curr_ym_value"]) - (sga_exp_curr_y_row?.["sga_amount_sum"] ?? 0))/(last_target.profit_target_amt * 100000000) * 100
        };
        ...
        const profit_rate_data =
        {
            "display_order": 8,
            "type": "영업이익률",
            "target_curr_y_value": (curr_target?.target_sale_amt ?? 0) === 0 ? 0 : (curr_target?.profit_target_amt ?? 0)/curr_target.target_sale_amt * 100,
            ...
            "actual_curr_ym_rate": (curr_target?.target_sale_amt ?? 0) === 0 || (pl_curr_y_row?.["sale_amount_sum"] ?? 0) === 0 ? 0 : (profit_data["actual_curr_ym_value"] / pl_curr_y_row?.["sale_amount_sum"])/((curr_target?.profit_target_amt ?? 0)/curr_target.target_sale_amt) * 100,
            "actual_last_ym_rate": (last_target?.target_sale_amt ?? 0) === 0 || (pl_last_y_row?.["sale_amount_sum"] ?? 0) === 0 ? 0 : (profit_data["actual_last_ym_value"] / pl_last_y_row?.["sale_amount_sum"])/((last_target?.profit_target_amt ?? 0)/last_target.target_sale_amt) * 100
        };
