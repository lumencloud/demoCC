module.exports = (srv) => {
    srv.on('get_forecast_dt_task_oi', async (req) => {
        ...
        const curr_pl = dt_pl_data.filter(o_pl => o_pl.year === year),
            ...
            dt_task_data.forEach(account => {
                let o_pl = curr_pl.find(pl => pl.dgtr_task_cd === account.dgtr_task_cd);
                let o_last_pl = last_pl.find(pl => pl.dgtr_task_cd === account.dgtr_task_cd);
                
                let o_curr_target = target_query.find(target => target.target_type_cd === account.dgtr_task_cd)
                if(!o_result[`${account.dgtr_task_cd}_sale`]){
                    o_result[`${account.dgtr_task_cd}_sale`]={display_order : account.sort_order, org_name : account.dgtr_task_nm, org_id : account.dgtr_task_cd, type: "매출"}
                }
                o_result[`${account.dgtr_task_cd}_sale`]['secured_value'] = (o_pl?.secured_sale ?? 0)
                o_result[`${account.dgtr_task_cd}_sale`]['not_secured_value'] = (o_pl?.not_secured_sale ?? 0)
                o_result[`${account.dgtr_task_cd}_sale`]['forecast_value'] = (o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)
                o_result[`${account.dgtr_task_cd}_sale`]['plan_ratio'] = ((o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)) - ((o_curr_target?.sale_target ?? 0)*100000000)
                o_result[`${account.dgtr_task_cd}_sale`]['yoy'] = ((o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)) - ((o_last_pl?.secured_sale ?? 0) + (o_last_pl?.not_secured_sale ?? 0))
