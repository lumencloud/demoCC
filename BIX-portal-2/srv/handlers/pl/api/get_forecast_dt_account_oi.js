module.exports = (srv) => {
    srv.on('get_forecast_dt_account_oi', async (req) => {
        ...
        const dt_pl_where_conditions = { 'year': {in:[year,last_year]}, 'dgtr_task_cd': {'!=':null, and : {'dgtr_task_cd': {'!=':''}}}, 'src_type': { '!=':'WO'}};
        ...
        let o_result = {}
        let o_total = {
            'sale':{"display_order": 1, "type": "매출", "org_name" : '합계', 'org_id':'total', target:0},
        }
        ...
        account_data.forEach(account => {
            ...
            if(!o_result[`${account.biz_tp_account_cd}_sale`]){
                o_result[`${account.biz_tp_account_cd}_sale`]={display_order : account.sort_order, org_name : account.biz_tp_account_nm, org_id : account.biz_tp_account_cd, type: "매출"}
            }
            ...
            o_result[`${account.biz_tp_account_cd}_sale`]['plan_ratio'] = ((o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)) - ((o_curr_target?.sale_target ?? 0)*100000000)
