        ...
        pl_data.forEach(o_pl=>{
            ...
            if(o_find_account){
                ...
                if(o_pl.year === year){
                    ...
                    o_data[`${o_pl.bd_n2_cd}`]['curr_margin_rate'] = (o_pl?.sale_amount_sum ?? 0) === 0 ? 0 : (o_pl?.margin_amount_sum ?? 0)/o_pl.sale_amount_sum*100;
                }else{
                    o_data[`${o_pl.bd_n2_cd}`]['last_sale'] = o_pl?.sale_amount_sum ?? 0;
                    o_data[`${o_pl.bd_n2_cd}`]['last_margin'] = o_pl?.margin_amount_sum ?? 0;
                    o_data[`${o_pl.bd_n2_cd}`]['last_margin_rate'] = (o_pl?.sale_amount_sum ?? 0) === 0 ? 0 : (o_pl?.margin_amount_sum ?? 0)/o_pl.sale_amount_sum*100;
                }
            }
        })
