module.exports = (srv) => {
    srv.on('get_forecast_dt_org_oi', async (req) => {
        ...
        let target_column = org_col_nm === 'div_id' ? [...target_col_list, 'hdqt_ccorg_cd as ccorg_cd','org_name'] : org_col_nm === 'hdqt_id' ? [...target_col_list, 'team_ccorg_cd as ccorg_cd','org_name'] : org_col_nm === 'team_id' ? [...target_col_list, 'team_ccorg_cd as ccorg_cd','org_name'] : [...target_col_list,'div_ccorg_cd as ccorg_cd','org_name']
        ...
        // DB 쿼리 실행 (병렬)
        const [dt_pl_data,target_query,lv1_target_data] = await Promise.all([
        ...
        let o_result = {};
        let o_total = {
            'sale':{"display_order": 1, "type": "매출", "org_name" : '합계','target':0},
        }
        ...
        target_query.forEach(o_target => {
            ...
            o_result[`${o_target.ccorg_cd}_sale`]['forecast_value'] = (o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)
            o_result[`${o_target.ccorg_cd}_sale`]['plan_ratio'] = ((o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0)) - ((o_target?.target_sale_amt ?? 0)*100000000)
            o_result[`${o_target.ccorg_cd}_sale`]['yoy'] = ((o_pl?.secured_sale ?? 0) + (o_pl?.not_secured_sale ?? 0))-((o_last_pl?.secured_sale ?? 0) + (o_last_pl?.not_secured_sale ?? 0))
