module.exports = (srv) => {
    srv.on('get_forecast_pl_pipeline_detail', async (req) => {
        ...
        let org_column = org_col_nm === 'div_id' ? ['hdqt_id as id','hdqt_name as name'] : org_col_nm === 'hdqt_id' || org_col_nm === 'team_id' ? ['team_id as id','team_name as name'] : ['div_id as id','div_name as name'];
        ...
        let org_groupBy = org_col_nm === 'div_id' ? ['hdqt_id','hdqt_name'] : org_col_nm === 'hdqt_id' || org_col_nm === 'team_id' ? ['team_id','team_name'] : ['div_id','div_name'];
        ...
        if(type === 'month'){
            ...
            const pl_groupBy_cols = ['org_order'];
            ...
            let o_total ={
                order:{display_order:1, itme_order:1, div_name:'합계', div_id:'total', type:'수주'},
                sale:{display_order:1, itme_order:2, div_name:'합계', div_id:'total', type:'매출'},
                count:{display_order:1, itme_order:3, div_name:'합계', div_id:'total', type:'건수'},
            }
            ...
            org_data.forEach(org => {
                ...
                if(!o_result[`${org.id}_order`]){
                    o_result[`${org.id}_order`]={display_order:(o_pl?.org_order ?? 0), itme_order:1, div_name:org.name, div_id:org.id, type:'수주',total_data:(o_pl?.rodr_amount_sum ?? 0)}
                    o_result[`${org.id}_sale`]={display_order:(o_pl?.org_order ?? 0), itme_order:2, div_name:org.name, div_id:org.id, type:'매출',total_data:(o_pl?.sale_amount_sum ?? 0)}
                    o_result[`${org.id}_cnt`]={display_order:(o_pl?.org_order ?? 0), itme_order:3, div_name:org.name, div_id:org.id, type:'건수',total_data:(o_pl?.total_rodr_cnt ?? 0)}
                }
                for(let i=12; i>Number(month); i--){
                    const s_index = i.toString().padStart(2,'0')
            ...
            const pl_where_conditions = {'year': year, 'src_type':'D'};
            const pl_groupBy_cols = [ 'deal_stage_cd', 'org_order'];
            ...
            let o_total ={
                order:{display_order:1, itme_order:1, div_name:'합계', div_id:'total', type:'수주',total_data:0},
                sale:{display_order:1, itme_order:2, div_name:'합계', div_id:'total', type:'매출',total_data:0},
                count:{display_order:1, itme_order:3, div_name:'합계', div_id:'total', type:'건수',total_data:0},
            }
            o_total['order'][`total_data`] = pl_data.reduce((iSum,oData)=>iSum += oData.rodr_amount_sum,0)
            o_total['sale'][`total_data`] = pl_data.reduce((iSum,oData)=>iSum += oData.sale_amount_sum,0)
            o_total['count'][`total_data`] = pl_data.reduce((iSum,oData)=>iSum += oData.total_rodr_cnt,0)
            ...
            org_data.forEach(org => {
                let o_pl = pl_data.find(pl => pl.id === org.id);
                if(!o_result[`${org.id}_order`]){
                    o_result[`${org.id}_order`]={display_order:(o_pl?.org_order ?? 0), itme_order:1, div_name:org.name, div_id:org.id, type:'수주',total_data:0}
                    o_result[`${org.id}_sale`]={display_order:(o_pl?.org_order ?? 0), itme_order:2, div_name:org.name, div_id:org.id, type:'매출',total_data:0}
                    o_result[`${org.id}_count`]={display_order:(o_pl?.org_order ?? 0), itme_order:3, div_name:org.name, div_id:org.id, type:'건수',total_data:0}
                ...
                if(o_pl?.deal_stage_cd ?? false){
                    let s_data_column = o_pl.deal_stage_cd.toLowerCase().replace('-','_').replace(' ','_')+'_data';
                    o_result[`${org.id}_order`][`${s_data_column}`] = o_pl.rodr_amount_sum
                    o_result[`${org.id}_sale`][`${s_data_column}`] = o_pl.sale_amount_sum
                    o_result[`${org.id}_count`][`${s_data_column}`] = o_pl.total_rodr_cnt
                    o_result[`${org.id}_order`]['total_data'] += o_pl.rodr_amount_sum
                    o_result[`${org.id}_sale`]['total_data'] += o_pl.sale_amount_sum
                    o_result[`${org.id}_count`]['total_data'] += o_pl.total_rodr_cnt
            ...
            let o_total ={
                order:{display_order:1, itme_order:1, div_name:'합계', div_id:'total', type:'수주', ...data_column},
                sale:{display_order:1, itme_order:2, div_name:'합계', div_id:'total', type:'매출', ...data_column},
                count:{display_order:1, itme_order:3, div_name:'합계', div_id:'total', type:'건수', ...data_column},
            }
            pl_data.forEach(pl => {
                for(let i=1; i<=12; i++){
            ...
            org_data.forEach(org => {
                let o_pl = pl_data.find(pl => pl.id === org.id);
                if(!o_result[`${org.id}_order`]){
                    o_result[`${org.id}_order`]={display_order:(o_pl?.org_order ?? 0), itme_order:1, div_name:org.name, div_id:org.id, type:'수주', ...data_column}
                    o_result[`${org.id}_sale`]={display_order:(o_pl?.org_order ?? 0), itme_order:2, div_name:org.name, div_id:org.id, type:'매출', ...data_column}
                    o_result[`${org.id}_cnt`]={display_order:(o_pl?.org_order ?? 0), itme_order:3, div_name:org.name, div_id:org.id, type:'건수', ...data_column}
                }
                for(let i=i_month+1; i<=12; i++){
                    let i_sale = o_pl?.[`m_${i}_sale_data`] ?? 0
                    let i_order = o_pl?.[`m_${i}_rodr_data`] ?? 0
