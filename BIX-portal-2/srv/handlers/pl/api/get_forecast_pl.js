module.exports = (srv) => {
    srv.on('get_forecast_pl', async (req) => {
        ...
        const sga_col_list = ['year', 'is_total_cc', s_secured, s_not_secured];
        ...
        const target_col_list = [
            ...
            'sum(ifnull(profit_target_amt,0)) as profit_target_amt'
        ];
        ...
        let target_column = org_col_nm === 'div_id' || org_col_nm === 'hdqt_id' ? [...target_col_list, 'max(ifnull(target_margin_rate,0)) as target_margin_rate','(sum(ifnull(target_sale_amt,0))*max(ifnull(target_margin_rate,0))/100) as target_margin'] : target_col_list
        ...
        let sga_curr_y_row = sga_query.filter(o => !o.is_total_cc && o.year === year),
            ...
            curr_target = query_target[0]
        
        
        const i_last_sga_forecast = sga_last_y_row.reduce((iSum, oData) => iSum += oData.secured_value, 0) + sga_last_y_row.reduce((iSum, oData) => iSum += oData.not_secured_value, 0),
            i_last_exp_sga_forecast = sga_exp_last_y_row.reduce((iSum, oData) => iSum += oData.secured_value, 0) + sga_exp_last_y_row.reduce((iSum, oData) => iSum += oData.not_secured_value, 0),
            i_last_sale_forecast = pl_last_y_data?.forecast_sale ?? 0,
            i_last_margin_forecast = pl_last_y_data?.forecast_margin ?? 0
        
        const o_sga_row = {
            forecast_value : sga_curr_y_row.reduce((iSum, oData) => iSum += oData.secured_value, 0) + sga_curr_y_row.reduce((iSum, oData) => iSum += oData.not_secured_value, 0),
            secured_value : sga_curr_y_row.reduce((iSum, oData) => iSum += oData.secured_value, 0),
            not_secured_value : sga_curr_y_row.reduce((iSum, oData) => iSum += oData.not_secured_value, 0),
        }
        const o_exp_sga_row = {
            ...
            not_secured_value : sga_exp_curr_y_row.reduce((iSum, oData) => iSum += oData.not_secured_value, 0),
        }
        ...
        const sale_data =
        {
            ...
            "plan_ratio": (pl_curr_y_data?.['forecast_sale'] ?? 0) - ((curr_target?.['target_sale_amt'] ?? 0) * 100000000),
        ...
        const margin_rate_data =
        {
            ...
            "plan_ratio": sale_data?.["forecast_value"] !== 0 ? ((margin_data?.["forecast_value"] / sale_data?.["forecast_value"] * 100) - (curr_target?.['target_margin_rate'] ?? 0)) : 0,
        ...
        const sga_data =
        {
            ...
            "not_secured_value": o_sga_row.not_secured_value,
            "plan_ratio": o_sga_row.forecast_value - (curr_target?.['sga_target_amt'] ?? 0),
            "yoy": o_sga_row.forecast_value - i_last_sga_forecast,
        };

        oResult.push(sga_data);

        // 공헌이익 [마진 - 사업SG&A] target_cont_margin_amt
        const contribution_data =
        {
            ...
            "not_secured_value": margin_data['not_secured_value'] - sga_data['not_secured_value'],
            "plan_ratio": margin_data['forecast_value'] - sga_data['forecast_value'] - (curr_target?.['target_cont_margin_amt'] ?? 0),
            "yoy": margin_data['forecast_value'] - sga_data['forecast_value'] - ((pl_last_y_data?.['forecast_margin'] ?? 0)-i_last_sga_forecast)
        };
        ...
        const sga_total_data =
        {
            "display_order": 6,
            "type": "전사 SG&A",
            "forecast_value": o_exp_sga_row.forecast_value,
            "secured_value": o_exp_sga_row.secured_value,
            "not_secured_value": o_exp_sga_row.not_secured_value,
            "plan_ratio": 0,
            "yoy": o_exp_sga_row.forecast_value - i_last_exp_sga_forecast
        };
        ...
        const profit_data =
        {
            ...
            "not_secured_value": contribution_data['not_secured_value'] - sga_total_data['not_secured_value'],
            "plan_ratio": contribution_data['forecast_value'] - sga_total_data['forecast_value'] - (curr_target?.profit_target_amt ?? 0),
        ...
        const profit_rate_data =
        {
            "display_order": 8,
            "type": "영업이익률",
            "forecast_value": sale_data['forecast_value'] === 0 ? 0 : profit_data['forecast_value']/sale_data['forecast_value']*100,
            "secured_value": sale_data['secured_value'] === 0 ? 0 : profit_data['secured_value']/sale_data['secured_value']*100,
            "not_secured_value": sale_data['not_secured_value'] === 0 ? 0 : profit_data['not_secured_value']/sale_data['not_secured_value']*100,
            "plan_ratio": 0,
            "yoy": (sale_data['forecast_value'] === 0 ? 0 : ((profit_data['forecast_value']/sale_data['forecast_value']*100))-(i_last_sale_forecast === 0 ? 0 : (i_last_margin_forecast-i_last_sga_forecast-i_last_exp_sga_forecast)/i_last_sale_forecast*100)),
        };
