module.exports = (srv) => {
    srv.on('get_forecast_br_detail', async (req) => {
        ...
        const target_col_list = [
            'year', 'org_id','sum(ifnull(br_target,0)) as br_target'];
        const target_where_conditions = {'is_total' : true, 'year': year, 'is_delivery':true};
        const target_groupBy_cols = ['year', 'org_id']

        // rsp 조회용 정보
        const rsp_col_list = ['month_amt', 'org_id','div_id','hdqt_id','team_id','div_name','hdqt_name','team_name', 'sum(ifnull(total_amt,0)) as total_amt', 'sum(ifnull(bill_amt,0)) as bill_amt'];
        const rsp_where_conditions = { 'year': year, 'is_delivery': true, 'team_id':{'!=':null} };
        const rsp_groupBy_cols = ['month_amt', 'org_id','div_id','hdqt_id','team_id','div_name','hdqt_name','team_name'];
        ...
        rsp_data.forEach(o_rsp =>{
            ...
            o_data['team'][`${o_rsp.team_id}`][`m_${Number(o_rsp.month_amt)}_data`] = (o_rsp?.total_amt ?? 0 ) === 0 ? 0 : (o_rsp?.bill_amt ?? 0)/(o_rsp.total_amt)*100
            o_data['sum'][`${o_rsp.div_id}`][`m_${Number(o_rsp.month_amt)}_total_amt`] = (o_data['sum'][`${o_rsp.div_id}`][`m_${Number(o_rsp.month_amt)}_total_amt`] || 0) + (o_rsp?.total_amt ?? 0 )
            o_data['sum'][`${o_rsp.div_id}`][`m_${Number(o_rsp.month_amt)}_bill_amt`] = (o_data['sum'][`${o_rsp.div_id}`][`m_${Number(o_rsp.month_amt)}_bill_amt`] || 0) + (o_rsp?.bill_amt ?? 0 )
            o_data['sum'][`${o_rsp.hdqt_id}`][`m_${Number(o_rsp.month_amt)}_total_amt`] = (o_data['sum'][`${o_rsp.hdqt_id}`][`m_${Number(o_rsp.month_amt)}_total_amt`] || 0) + (o_rsp?.total_amt ?? 0 )
            o_data['sum'][`${o_rsp.hdqt_id}`][`m_${Number(o_rsp.month_amt)}_bill_amt`] = (o_data['sum'][`${o_rsp.hdqt_id}`][`m_${Number(o_rsp.month_amt)}_bill_amt`] || 0) + (o_rsp?.bill_amt ?? 0 )
        })
        ...
        a_data.forEach(o_data =>{
            ...
            if(!s_div_check && org_col_nm !== 'hdqt_id' && org_col_nm !== 'team_id'){
                ...
                for(i=1; i<13; i++){
                    o_temp[`m_${i}_data`] = o_sum[`m_${i}_total_amt`] === 0 ? 0 : o_sum[`m_${i}_bill_amt`] / o_sum[`m_${i}_total_amt`] * 100
                }
