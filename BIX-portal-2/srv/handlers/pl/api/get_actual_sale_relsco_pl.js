module.exports = (srv) => {
    srv.on('get_actual_sale_relsco_pl', async (req) => {
        ...
        let o_data = {}
        let o_total = {
            sale_true: {"display_order": i_count,"org_nm":'합계',"relsco_type": "대내","type": "매출"},
            margin_true: {"display_order": ++i_count,"org_nm":'합계',"relsco_type": "대내","type": "마진"},
            margin_rate_true: {"display_order": ++i_count,"org_nm":'합계',"relsco_type": "대내","type": "마진율"},
            sale_false: {"display_order": ++i_count,"org_nm":'합계',"relsco_type": "대외","type": "매출"},
            margin_false: {"display_order": ++i_count,"org_nm":'합계',"relsco_type": "대외","type": "마진"},
            margin_rate_false: {"display_order": ++i_count,"org_nm":'합계',"relsco_type": "대외","type": "마진율"},
        }; 
        ...
        org_list.forEach((data)=>{
            ...
            a_pl_data.forEach((o_pl_data,i)=>{
                if(data.id === o_pl_data.id){
                    ...
                    margin_rate_true_data["actual_curr_ym_value"] = (o_pl_data?.["curr_true_sale"] ?? 0) === 0 ? 0 : (o_pl_data?.["curr_true_margin"] ?? 0)/o_pl_data["curr_true_sale"]*100;
                    margin_rate_true_data["actual_last_ym_value"] = (o_pl_data?.["last_true_sale"] ?? 0) === 0 ? 0 : (o_pl_data?.["last_true_margin"] ?? 0)/o_pl_data["last_true_sale"]*100;
                    ...
                    margin_rate_false_data["actual_curr_ym_value"] = (o_pl_data?.["curr_false_sale"] ?? 0) === 0 ? 0 : (o_pl_data?.["curr_false_margin"] ?? 0)/o_pl_data["curr_false_sale"]*100;
                    margin_rate_false_data["actual_last_ym_value"] = (o_pl_data?.["last_false_sale"] ?? 0) === 0 ? 0 : (o_pl_data?.["last_false_margin"] ?? 0)/o_pl_data["last_false_sale"]*100;
