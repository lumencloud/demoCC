module.exports = (srv) => {
    srv.on('get_forecast_pl_pipeline_account', async (req) => {
        ...
        if (type === 'month') {
            ...
            const pl_col_list = [
                'biz_tp_account_cd',
                `sum(case when src_type = 'D' then ifnull(sale_year_amt,0) else 0 end) as sale_amount_sum`,
                `sum(case when src_type = 'D' then ifnull(rodr_year_amt,0) else 0 end) as rodr_amount_sum`,
            ];
            ...
            pl_data.forEach(o_pl => {
                ...
                if (o_account) {
                    if (!o_result[`${o_pl.biz_tp_account_cd}_order`]) {
                        o_result[`${o_pl.biz_tp_account_cd}_order`] = { display_order: o_account.sort_order, item_order: 1, div_name: o_account.biz_tp_account_nm, div_id: o_account.biz_tp_account_cd, type: '수주', total_data: 0 }
                        o_result[`${o_pl.biz_tp_account_cd}_sale`] = { display_order: o_account.sort_order, item_order: 2, div_name: o_account.biz_tp_account_nm, div_id: o_account.biz_tp_account_cd, type: '매출', total_data: 0 }
                        o_result[`${o_pl.biz_tp_account_cd}_count`] = { display_order: o_account.sort_order, item_order: 3, div_name: o_account.biz_tp_account_nm, div_id: o_account.biz_tp_account_cd, type: '건수', total_data: 0 }
            ...
            for (let i = 1; i <= 12; i++) {
                pl_col_list.push(`(case when src_type = 'D' then ifnull(sale_m${i}_amt,0) else 0 end) as m_${i}_sale_data`)
                pl_col_list.push(`(case when src_type = 'D' then ifnull(rodr_m${i}_amt,0) else 0 end) as m_${i}_rodr_data`)
            }
            const pl_where_conditions = { 'year': year, 'src_type': {'!=':'WO'} };
            let pl_where = org_col_nm === 'lv1_id' ? pl_where_conditions : { ...pl_where_conditions, [org_col_nm]: org_id };
