module.exports = (srv) => {
    srv.on('get_actual_dt_org_oi', async (req) => {
        ...
        const pl_col_list = ['year', sale_sum_col];
        const target_col = ['org_id', 'target_year', 'org_name', 'dt_target_sale_amt'];
         ...
         */
        const pl_where_conditions = { 'year': { in: [year, last_year]}, 'dgtr_task_cd': {'!=':null, and : {'dgtr_task_cd': {'!=':''}}}, src_type:{ in : ['E','WO','P']} };
        ...
        let total_data = [
            {
                ...
                "org_id": "total",
                "org_name": '합계',
                "target_curr_y_value": org_col_nm === 'lv1_id' ? Number((oCurrTargetEnt?.dt_target_sale_amt ?? 0)) : (org_col_nm === 'div_id' || org_col_nm === 'hdqt_id') ? Number((flat_target?.[`_${org_id}_${year}_dt_target_sale_amt`] ?? 0)) : 0,
        ...
        org_list.forEach((data,i)=>{
            ...
            pl_data.forEach(data2=>{
                if(data.id === data2[search_org]){
                    sale_data["target_curr_y_value"]= Number((flat_target?.[`_${data.id}_${year}_dt_target_sale_amt`] ?? 0));
                    sale_data["actual_curr_ym_value"]= flat_pl?.[`_${data.id}_${year}_sale_amount_sum`] ?? 0;
                    sale_data["actual_last_ym_value"]= flat_pl?.[`_${data.id}_${last_year}_sale_amount_sum`] ?? 0;
                    sale_data["actual_curr_ym_rate"]= Number((flat_target?.[`_${data.id}_${year}_dt_target_sale_amt`] ?? 0)) === 0 ? 0 : (flat_pl?.[`_${data.id}_${year}_sale_amount_sum`] ?? 0) / (Number((flat_target?.[`_${data.id}_${year}_dt_target_sale_amt`] ?? 0))*100000000) * 100;
                    sale_data["actual_last_ym_rate"]= Number((flat_target?.[`_${data.id}_${last_year}_dt_target_sale_amt`] ?? 0)) === 0 ? 0 : (flat_pl?.[`_${data.id}_${last_year}_sale_amount_sum`] ?? 0) / (Number((flat_target?.[`_${data.id}_${last_year}_dt_target_sale_amt`] ?? 0))*100000000) * 100;
                };
                if(i === 0){
                    if(data2.year === year){
                        total_data[0].actual_curr_ym_value += data2.sale_amount_sum;
                    }else if(data2.year === last_year){
                        total_data[0].actual_last_ym_value += data2.sale_amount_sum;
                    };
                };
            });
        ...
        if(org_col_nm === 'lv1_id'){
            iLastYearTotalTarget = Number(oLastTargetEnt?.dt_target_sale_amt ?? 0);
        }else if(org_col_nm === 'lv2_id' || org_col_nm === 'lv3_id'){
            target_data.forEach(data => {
                if(data.target_year === year){
                    total_data[0].target_curr_y_value += Number(data?.dt_target_sale_amt ?? 0);
                }else if(data.target_year === last_year){
                    iLastYearTotalTarget += Number(data?.dt_target_sale_amt ?? 0);
                };
            });
        }else if(org_col_nm === 'div_id' || org_col_nm === 'hdqt_id'){
            iLastYearTotalTarget = Number((flat_target?.[`_${org_id}_${last_year}_dt_target_sale_amt`] ?? 0));
        };

        total_data[0].actual_curr_ym_rate = total_data[0].target_curr_y_value === 0 ? 0 : total_data[0].actual_curr_ym_value / (total_data[0].target_curr_y_value *100000000) * 100;
        total_data[0].actual_last_ym_rate = iLastYearTotalTarget === 0 ? 0 : total_data[0].actual_last_ym_value / (iLastYearTotalTarget*100000000) * 100;
        total_data[0].actual_ym_rate_gap = total_data[0].actual_curr_ym_rate - total_data[0].actual_last_ym_rate;
