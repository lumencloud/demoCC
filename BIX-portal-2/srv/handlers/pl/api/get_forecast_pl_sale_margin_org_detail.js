module.exports = (srv) => {
    srv.on('get_forecast_pl_sale_margin_org_detail', async (req) => {
        ...
        let target_column = org_col_nm === 'div_id' ? [...target_col_list, s_margin_column,'hdqt_ccorg_cd as ccorg_cd','org_name'] : org_col_nm === 'hdqt_id' ? [...target_col_list, s_margin_column,'team_ccorg_cd as ccorg_cd','org_name'] : org_col_nm === 'team_id' ? [...target_col_list, s_margin_column,'team_ccorg_cd as ccorg_cd','org_name'] : [...target_col_list,'div_ccorg_cd as ccorg_cd','org_name']
        ...
        // DB 쿼리 실행 (병렬)
        const [query, target_query,lv1_target_data] = await Promise.all([
            // PL 실적, 목표 조회
        ...
        target_query.forEach(o_target => {
            let o_pl = curr_pl.find(pl => pl.ccorg_cd === o_target.ccorg_cd);
            let o_last_pl = last_pl.find(pl => pl.ccorg_cd === o_target.ccorg_cd);
            
            if(!o_result[`${o_target.ccorg_cd}_sale`]){
                o_result[`${o_target.ccorg_cd}_sale`] = {
                    "display_order": o_target.org_order,
                    "item_order" : 1,
                    "type": "매출",
                    "org_name" : o_target.org_name,
                    "target" : o_target?.target_sale_amt ?? 0
                }
                o_result[`${o_target.ccorg_cd}_margin`] = {
                    "display_order": o_target.org_order,
                    "item_order" : 2,
                    "type": "마진",
                    "org_name" : o_target.org_name,
            ...
            o_result[`${o_target.ccorg_cd}_sale`]['secured_value'] = o_pl?.secured_sale ?? 0
            o_result[`${o_target.ccorg_cd}_margin`]['secured_value'] = o_pl?.secured_margin ?? 0
            o_result[`${o_target.ccorg_cd}_sale`]['not_secured_value'] = o_pl?.not_secured_sale ?? 0
            o_result[`${o_target.ccorg_cd}_margin`]['not_secured_value'] = o_pl?.not_secured_margin ?? 0
            o_result[`${o_target.ccorg_cd}_sale`]['forecast_value'] = o_pl?.forecast_sale ?? 0
            o_result[`${o_target.ccorg_cd}_margin`]['forecast_value'] = o_pl?.forecast_margin ?? 0
            o_result[`${o_target.ccorg_cd}_sale`]['last_forecast_value'] = o_last_pl?.forecast_sale ?? 0
            o_result[`${o_target.ccorg_cd}_margin`]['last_forecast_value'] = o_last_pl?.forecast_margin ?? 0

            o_total[`sale`]['target'] += (o_target?.target_sale_amt ?? 0)
            o_total[`margin`]['target'] += (o_target?.target_margin ?? 0)
        })

        if(org_col_nm === 'lv1_id'){
            o_total[`sale`]['target'] = lv1_target_data[0].target_sale_amt
