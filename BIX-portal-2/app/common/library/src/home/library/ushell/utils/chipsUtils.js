...
sap.ui.define([
    ...
    let DYNAMIC_BASE_CHIP_ID = "X-SAP-UI2-CHIP:/UI2/DYNAMIC_APPLAUNCHER",
    ...
    chipsUtils.getCatalogTilePreviewTitle = function (oCatalogTile) {
        let sAppLauncherTitle = chipsUtils.getBagText(oCatalogTile, "tileProperties", "display_title_text");
    ...
    chipsUtils.getCatalogTilePreviewSubtitle = function (oCatalogTile) {
        let sAppLauncherSubTitle = chipsUtils.getBagText(oCatalogTile, "tileProperties", "display_subtitle_text");
    ...
    chipsUtils.getCatalogTilePreviewIcon = function (oCatalogTile) {
        let sAppLauncherIcon = chipsUtils._getConfigurationProperty(oCatalogTile, "tileConfiguration", "display_icon_url");
    ...
    chipsUtils.getCatalogTileTargetURL = function (oCatalogTile) {
        let sAppLauncherTarget = chipsUtils._getConfigurationProperty(oCatalogTile, "tileConfiguration", "navigation_target_url");
    ...
    chipsUtils.getCatalogTilePreviewIndicatorDataSource = function (oCatalogTile) {
        ...
        let oTileConfiguration = chipsUtils._getAppLauncherTileConfiguration(oCatalogTile);
    ...
    chipsUtils.getTileSize = function (oTile) {
        let row = (!oTile.isStub() && oTile.getConfigurationParameter("row")) || "1",
    ...
    chipsUtils._isAppLauncher = function (oChipInstance) {
        let sBaseChipId = oChipInstance.getChip().getBaseChipId();
    ...
    chipsUtils.getTargetUrlFromSimplifiedChip = function (oSimplifiedChip, URLParsing) {
        let oCustomTileTarget = this.getCustomTileTargetFromSimplified(oSimplifiedChip);
        if (oCustomTileTarget) {
            return "#" + URLParsing.constructShellHash(oCustomTileTarget);
        }

        let oTileConfiguration = this.getTileConfigurationFromSimplifiedChip(oSimplifiedChip);
    ...
    chipsUtils.getIndicatorDataSourceFromSimplifiedChip = function (oSimplifiedChip) {
        ...
        let sServiceUrl = oTileConfiguration.service_url;
    ...
    chipsUtils.getTileSizeFromSimplifiedChip = function (oSimplifiedChip) {
        let oConfiguration = oSimplifiedChip.configuration;
        let sRow = oConfiguration.row || 1;
        let sCol = oConfiguration.col || 1;
    ...
    chipsUtils.getInfoFromSimplifiedChip = function (oSimplifiedChip) {
        let oTileProperties = oSimplifiedChip.bags.tileProperties;
    ...
    chipsUtils.getKeywordsFromSimplifiedChip = function (oSimplifiedChip) {
        let oTileProperties = oSimplifiedChip.bags.tileProperties;
        let sKeywordsText = oTileProperties && oTileProperties.texts.display_search_keywords;
    ...
    chipsUtils.isCustomTileFromSimplifiedChip = function (oSimplifiedChip) {
        let sChipId = oSimplifiedChip.chipId;
    ...
    chipsUtils.loadChipInstanceFromSimplifiedChip = function (oSimplifiedChip) {
        let oBags = oSimplifiedChip.bags;
        let oRawData = {
        ...
        return Container.getServiceAsync("PageBuilding")
            .then(function (oPageBuildingService) {
                ...
                let oChipInstance = oFactory.createChipInstance(oRawData);
    ...
    chipsUtils.addBagDataToChipInstance = function (oChipInstance, oBags) {
        ...
        for (let sBagId in oBags) {
            let oBagData = oBags[sBagId];
            let oBag = oChipInstance.getBag(sBagId);
    ...
    chipsUtils.getCustomTileTargetFromSimplified = function (oSimplifiedChip) {
        ...
        // Smart Business tile
        let oTileConfiguration = JSON.parse(oSimplifiedChip.configuration.tileConfiguration || "{}");
        let oTileProperties = JSON.parse(oTileConfiguration.TILE_PROPERTIES || "{}");
