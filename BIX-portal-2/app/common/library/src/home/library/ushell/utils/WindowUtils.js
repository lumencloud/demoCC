...
sap.ui.define([
    ...
    WindowUtils.hasInvalidProtocol = function (URL) {
        ...
        let oURL = document.createElement("a");
    ...
    WindowUtils.openURL = function (URL, windowName, windowFeatures, safeMode) {
        let bSafeMode = (safeMode === undefined) ? true : safeMode;
        let bIsCrossOriginUrl = this.isCrossOriginUrl(URL);
    ...
    WindowUtils.getLeanURL = function (targetURL, navigationURL) {
        let sUrl = targetURL || navigationURL;
     ...
     *
     * @param {string[]} aUrlParams Array of objects with url parameters and their value which should be added to the url
    ...
    WindowUtils._getAdjustedQueryString = function (aUrlParams, aObsoleteUrlParams) {
        ...
        let oOriginUriParameters = new URLSearchParams(window.location.search);
        let oNewUrlParameter = Array.from(oOriginUriParameters.keys()).reduce(function (oResult, sParam) {
        ...
        if (aUrlParams && aUrlParams.length > 0) {
            aUrlParams.forEach(function (oParam) {
                let sParamName = Object.keys(oParam)[0];
                if (sParamName) {
    ...
    WindowUtils.refreshBrowser = function (aUrlParams, aObsoleteUrlParams) {
        let sNewHref = ushellUtils.getLocationHref();
        let sLocationSearch = ushellUtils.getLocationSearch();
        let sNewLocationSearch = WindowUtils._getAdjustedQueryString(aUrlParams, aObsoleteUrlParams);
