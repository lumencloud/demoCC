...
sap.ui.define([
	 ...
	 */
	let Element = ManagedObject.extend("sap.ui.core.Element", {
	...
	Element.prototype.prop = function(sPropertyName, oValue) {
		...
		let oPropertyInfo = this.getMetadata().getAllSettings()[sPropertyName];
		if (oPropertyInfo) {
	...
	Element.prototype.setProperty = function(sPropertyName, vValue, bSuppressInvalidate) {
		...
		if (sPropertyName == "enabled") {
			let bOldEnabled = this.mProperties.enabled;
	...
	*/
	...
	Element.prototype.destroy = function(bSuppressInvalidate) {
		...
		let bHasNoParent = !this.getParent();
		...
		if (needSyncDomRemoval.call(this, bSuppressInvalidate, bHasNoParent)) {
			...
			if (bSuppressInvalidate !== "KeepDom") {
				...
				for (let i = 0, aDomRefs = oDomRef.querySelectorAll('[id^="' + this.getId() + '-"]'); i < aDomRefs.length; i++) {
	...
	Element.prototype.isFocusable = function() {
		let oFocusDomRef = this.getFocusDomRef();

		if (!oFocusDomRef) {
			return false;
		}

		let oCurrentDomRef = oFocusDomRef;
		let aViewport = [[0, window.innerWidth], [0, window.innerHeight]];
		...
		while (!aIntersectionX || !aIntersectionY) {
			let oRect = oCurrentDomRef.getBoundingClientRect();
		...
		let aElements = document.elementsFromPoint(
			Math.floor((aIntersectionX[0] + aIntersectionX[1]) / 2),
			Math.floor((aIntersectionY[0] + aIntersectionY[1]) / 2)
		);

		let iFocusDomRefIndex = aElements.findIndex(function(oElement) {
			return oElement.contains(oFocusDomRef);
		});

		let iBlockLayerIndex = aElements.findIndex(function(oElement) {
	...
	Element.prototype._refreshTooltipBaseDelegate = function (oTooltip) {
		let oOldTooltip = this.getTooltip();
	 ...
	 * @synthetic
	 */
	let CustomData = Element.extend("sap.ui.core.CustomData", /** @lends sap.ui.core.CustomData.prototype */ { metadata : {
	...
	CustomData.prototype.setValue = function(oValue) {
		...
		if (oControl && oControl.getDomRef()) {
			let oCheckResult = this._checkWriteToDom(oControl);
			if (oCheckResult) {
				// update DOM directly
	...
	CustomData.prototype._checkWriteToDom = function(oRelated) {
		...
		let ID = DataType.getType("sap.ui.core.ID");
	...
	function findCustomData(element, key) {
		let aData = element.getAggregation("customData");
		if (aData) {
	...
	function setCustomData(element, key, value, writeToDom) {
		let oDataObject = findCustomData(element, key);

		if (value === null) { // delete this property
			if (!oDataObject) {
				return;
			}
			let dataCount = element.getAggregation("customData").length;
			if (dataCount == 1) {
	...
	Element.prototype.data = function() {
		...
		if (argLength == 0) {                    // return ALL data as a map
			let aData = this.getAggregation("customData"),
				result = {};
			if (aData) {
				for (let i = 0; i < aData.length; i++) {
			...
			if (arg0 === null) {                  // delete ALL data
				...
				let dataObject = findCustomData(this, arg0);
	...
	Element.prototype.clone = function(sIdSuffix, aLocalIds){
		...
		let oClone = ManagedObject.prototype.clone.apply(this, arguments);
		// Clone delegates
		for ( let i = 0; i < this.aDelegates.length; i++) {
			if (this.aDelegates[i].bClone) {
				oClone.aDelegates.push(this.aDelegates[i]);
			}
		}
		for ( let k = 0; k < this.aBeforeDelegates.length; k++) {
	...
	function fireLayoutDataChange(oElement) {
		let oLayout = oElement.getParent();
		if (oLayout) {
			let oEvent = jQuery.Event("LayoutDataChange");
	...
	Element.prototype.getDomRefForSetting = function (sSettingsName) {
		let oSetting = this.getMetadata().getAllSettings()[sSettingsName];
		if (oSetting && oSetting.selector) {
			let oDomRef = this.getDomRef();
			if (oDomRef) {
				oDomRef = oDomRef.parentNode;
				if (oDomRef && oDomRef.querySelector ) {
					let sSelector = oSetting.selector.replace(/\{id\}/g, this.getId().replace(/(:|\.)/g,'\\$1'));
	...
	Element.prototype._getCurrentMediaContainerRange = function (sName) {
		let iWidth = this._getMediaContainerWidth();
	...
	Element.prototype._onContextualSettingsChanged = function () {
		...
		aListeners.forEach(function (oL) {
			let oMedia = this._getCurrentMediaContainerRange(oL.name);
	...
	Element.prototype._detachMediaContainerWidthChange = function (fnFunction, oListener, sName) {
		...
		for (let i = 0, iL = this._aContextualWidthListeners.length; i < iL; i++) {
	...
	Element.getActiveElement = () => {
		try {
			let $Act = jQuery(document.activeElement);
			if ($Act.is(":focus")) {
	...
	Theming.attachApplied(function(oEvent) {
		...
		let oJQueryEvent = jQuery.Event("ThemeChanged");
