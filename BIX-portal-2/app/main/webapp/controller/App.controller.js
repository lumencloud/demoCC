sap.ui.define([
  ...
  Popover, Button, library, Filter, FilterOperator, Component, ComponentContainer, EventBus, HashChanger) => {
  "use strict";
  ...
  return BaseController.extend("bix.main.controller.App", {
    ...
    onBeforeRendering() {
      ...
      let sIconKey, sNavKey;
      if (!sessionStorage.getItem("/seletedNavTab")) {
        sIconKey = sessionStorage.getItem("/seletedIconTab")
        this.byId("iconTab").setSelectedKey(sIconKey);
      } else {
        sIconKey = sessionStorage.getItem("/seletedIconTab")
        sNavKey = sessionStorage.getItem("/seletedNavTab")
        this.byId("iconTab").setSelectedKey(sIconKey);
        this.byId("navList").setSelectedKey(sNavKey);
      }
    ...
    onNavToApp(oEvent) {
      ...
      const oUiModel = this.getOwnerComponent().getModel("tabState");
      ...
      if (oInfo.category === 'home') {
        ...
        sessionStorage.setItem("/seletedIconTab", oInfo.ID)
        oChatbotBtn.addStyleClass("custom-dashboard-version")
      ...
      if (sCode) {
        ...
        if (bCheck) {
          let sSelectedKey = oInfo.Parent_ID || oInfo.ID
          this.getOwnerComponent().getRouter().navTo(sRouteName, { tab: oInfo }, oComponentTargetInfo);
          oModel.setProperty("/sideVisible", false);
          oModel.setProperty("/sideSelectedkey", sSelectedKey)
          oUiModel.setProperty("/selectedTab", sSelectedKey);

          if (oInfo.Parent_ID) {
            sessionStorage.setItem("/seletedIconTab", oInfo.Parent_ID)
            sessionStorage.setItem("/seletedNavTab", oInfo.ID)
          } else {
            sessionStorage.setItem("/seletedIconTab", oInfo.ID)
          }
          oChatbotBtn.removeStyleClass("custom-dashboard-version")
