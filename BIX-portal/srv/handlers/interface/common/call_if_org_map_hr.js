...
module.exports = (srv) => {
    srv.on('call_if_org_map', async (req) => {

        // cds 모듈을 통한 DB 커넥트
        const db = await cds.connect.to('db');
        const common_if_org_map = db.entities('common').if_org_map;
        ...
        if(response.data.returnCode === 'S'){
            ...
            try {
                ...
                const procQuery = `CALL P_IF_COMMON_ORG_MAP_HR_TRSF(O_RESULT => ?)`;
                ...
                if(O_RESULT.O_RESULT.length > 0 && O_RESULT.O_RESULT.find(o=>o.RESULT_CODE === 'NG') ){
                    await DELETE.from(common_if_org_map).where({flag : null});
                    await tx.rollback();
