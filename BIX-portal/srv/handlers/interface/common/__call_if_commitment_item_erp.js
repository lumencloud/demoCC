...
module.exports = (srv) => {
    srv.on('call_if_commitment_item_erp', async (req) => {
        ...
        try {
            ...
            const requestData = async (sURL)=>{
                ...
                await tx.run(`CALL P_IF_COMMON_COMMITMENT_ITEM_ERP_RCV( VER => '${ver}', JSONDATA => ?, O_RESULT => ? )`, [IF_STRING_JSON]);
            }

            const entityUri = '/sap/opu/odata4/sap/zapi_bix_master_o4/srvd_a2x/sap/zsd_bix_master/0001/CommitmentItem';
            const call_list = [];
            ...
            const procQuery = `CALL P_IF_COMMON_COMMITMENT_ITEM_ERP_TRSF( VER => '${ver}', O_RESULT => ? )`;
