    ...
    srv.on('call_if_gl_account_erp', async (req) => {
        ...
        try {
            ...
            const requestData = async (sURL)=>{
                ...
                await tx.run(`CALL P_IF_COMMON_GL_ACCOUNT_ERP_RCV( VER => '${ver}', JSONDATA => ?, O_RESULT => ? )`, [IF_STRING_JSON]);
            }
            ...
            const procQuery = `CALL P_IF_COMMON_GL_ACCOUNT_ERP_TRSF( VER => '${ver}', O_RESULT => ? )`;
