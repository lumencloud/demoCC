module.exports = (srv) => {
    srv.on('call_if_prj_labor_promis', async (req) => {

        // cds 모듈을 통한 DB 커넥트
        const db = await cds.connect.to('db');
        const oi_if_ito = db.entities('oi').if_ito;
        ...
        if(response.data.returnCode === 'S'){
            ...
            try {
                ...
                const procQuery = `CALL P_IF_RSP_PRJ_LABOR_PROMIS_TRSF(O_RESULT => ?)`;
                ...
                if(O_RESULT.O_RESULT.length > 0 && O_RESULT.O_RESULT.find(o=>o.RESULT_CODE === 'NG') ){
                    await DELETE.from(oi_if_ito).where({flag : null});
                    await tx.rollback();
