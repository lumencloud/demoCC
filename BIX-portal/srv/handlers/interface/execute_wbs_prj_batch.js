...
module.exports = (srv) => {
    srv.on('execute_wbs_prj_batch', async (req) => {
        const { ver, ver_last, trsf } = req.data;
        ...
        const empty_prj = await SELECT.distinct.from(pl_if_wideview).columns('prj_no')
            .where({ prj_no: { 'not in': SELECT.distinct.from(common_if_project).columns('prj_no').where({ 'ver': ver }) }, and: { 'ver': ver, or: { 'ver': ver_last } } });
        ...
        try {
            ...
            const requestData = async (sURL) => {
                ...
                await tx.run(`CALL P_IF_COMMON_PROJECT_WBS_ERP_RCV( VER => '${ver}', JSONDATA => ? )`, [IF_STRING_JSON])
                    .catch(err => {
                        Insert_log('RCV', 'ERP', 'COMMON_IF_PROJECT', "P_IF_COMMON_PROJECT_WBS_ERP_RCV", false, err.message?.slice(0, 500), err.code);
            ...
            // trsf true 일 경우 본테이블 이관 프로시저까지 한번에 처리
            if (trsf) await tx.run(`CALL P_IF_COMMON_PROJECT_PROMIS_TRSF( VER => '${ver}', O_RESULT => ?)`);
