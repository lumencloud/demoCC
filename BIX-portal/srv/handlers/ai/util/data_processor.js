...
class DataProcessor {
  ...
  processInteractionData(interactionData) {
    try {
      ...
      this.logger.info('interactionData(확인):', interactionData);
  ...
  loadMenuListData() {
    try {
        const menuListPath = paths.MENU_LIST_PATH;
        const rawData = fs.readFileSync(menuListPath, 'utf8');
        const menuList = JSON.parse(rawData);
        
        // url 항목 제거
        return menuList.map(menu => {
            const { url, ...menuWithoutUrl } = menu;
            return menuWithoutUrl;
        });
    } catch (error) {
        console.error('[DataProcessor] 메뉴 리스트 로드 오류:', error);
        return [];
    }
  }

  /**
   * selected_menus ID를 메뉴 정보로 매핑
   * @param {string} executiveSummary - 네비게이터 에이전트 결과
   * @returns {string} 매핑된 정보가 포함된 결과
   */
  enhanceNavigatorResult(executiveSummary) {
    try {
        const summaryData = JSON.parse(executiveSummary);
        
        if (!summaryData.selected_menus || !Array.isArray(summaryData.selected_menus)) {
        ...
        const menuListPath = paths.MENU_LIST_PATH;
        const rawData = fs.readFileSync(menuListPath, 'utf8');
        const fullMenuList = JSON.parse(rawData);
        
        // ID를 객체로 변환
        const enhancedScreens = summaryData.selected_menus.map(screenId => {
            const menuItem = fullMenuList.find(menu => menu.menu_id === screenId);
            return {
                id: screenId,
                name: menuItem?.name || screenId,
        ...
        return JSON.stringify({
            ...summaryData,
            selected_menus: enhancedScreens
        });

    } catch (error) {
        console.error('네비게이터 결과 향상 오류:', error);
        return executiveSummary;
    }
  }
