...
class AgentExecutor {
    constructor() {
        ...
        this.logger.info(`초기화 완료. 등록된 에이전트 수: ${this.agentRegistry.size}`);
        this.logger.debug('등록된 에이전트 목록', { agents: Array.from(this.agentRegistry.keys()) });
    }
    ...
    loadAgentsFromPaths() {
        // 일반질의 에이전트 로드
        this.loadAgentFromFile('general_qa_agent', paths.GENERAL_QA_AGENT_PATH);
    ...
    loadAgentFromFile(expectedAgentId, filePath) {
        try {
            this.logger.debug(`에이전트 로드 시도: ${expectedAgentId}`, { filePath });

            // 파일 존재 확인
            if (!fs.existsSync(filePath)) {
                this.logger.warn(`에이전트 파일 없음: ${expectedAgentId}`, { filePath });
                return;
            }
            ...
            this.logger.info(`에이전트 등록 완료: ${expectedAgentId}`);
        } catch (error) {
            this.logger.error(`에이전트 로드 실패: ${expectedAgentId}`, error);
        }
    }
        ...
        for (const step of sortedSteps) {
            try {
                ...
                if (component.type === 'prompt') {
                    stepResult = await this._executePromptComponent(component, componentInputs, inputs);
                }
     ...
     * @private
     */
    async _executePromptComponent(component, inputs, executionContext) {
        if (!this.llmService) {
