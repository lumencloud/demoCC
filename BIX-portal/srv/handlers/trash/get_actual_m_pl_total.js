module.exports = (srv) => {
    srv.on('get_actual_m_pl_total', async (req) => {
        try{
            //
            // function 호출 리턴 객체
            const aRes = [];
             ...
             * 최상위 조직 정보 반환
             */
            const orgInfo = await SELECT.one.from(org_full_level).columns(['org_ccorg_cd']).where({ org_id: { '!=': null }, org_parent: null });
            if (!orgInfo) return '조직 조회 실패'; // 화면 조회 시 유효하지 않은 조직코드 입력시 예외처리 추가 필요 throw error

            let pl_col_list = ['src_type'];
            let pl_where_conditions = { year: year, lv1_ccorg_cd: orgInfo.org_ccorg_cd, src_type: { '!=': 'WA' } };
