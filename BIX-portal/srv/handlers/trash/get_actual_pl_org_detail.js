...
//
             ...
             */
            const pl_col_list = [
                'year', 'month_amt', 'sum(sale_amount_sum) as sale_amount_sum', 'sum(prj_prfm_amount_sum) as prj_prfm_amount_sum', 'ifnull(sum(margin_amount_sum), 0) as margin_amount_sum'];
            ...
            const sga_col_list = ['year', 'month_amt',
                'ifnull(sum(labor_amount_sum),0) + ifnull(sum(iv_amount_sum),0) + ifnull(sum(exp_amount_sum),0) as amount_sum'];
            ...
            if (!query.length && !query_target.length && !sga_query.length && !rsp_query.length) {
            ...
            org_data.forEach(data =>{
                if(data[`${org_chk}`]){
                    ...
                    let br_curr_ym_value = (flat_rsp?.[`_`+id_value+`_`+year+`total_year_amt`] ?? 0) !== 0 ? ((flat_rsp?.[`_`+id_value+`_`+year+`bill_year_amt`] ?? 0) / (flat_rsp[`_`+id_value+`_`+year+`total_year_amt`])) * 100 : 0;
                    let br_last_ym_value = (flat_rsp?.[`_`+id_value+`_`+last_year+`total_year_amt`] ?? 0)  !== 0 ? ((flat_rsp?.[`_`+id_value+`_`+last_year+`bill_year_amt`] ?? 0) / (flat_rsp[`_`+id_value+`_`+last_year+`total_year_amt`])) * 100 : 0;
                    oTemp = {
                        type : org_chk,
                        rowType : "br",
                        seq : 8,
                    ...
                    let rohc_curr_ym_value = (flat_rsp?.[`_`+id_value+`_`+year+`total_year_amt`] ?? 0) !== 0 ? (margin_curr_ym_value - sga_curr_ym_value) / flat_rsp[`_`+id_value+`_`+year+`total_year_amt`] : 0;
                    let rohc_last_ym_value = (flat_rsp?.[`_`+id_value+`_`+last_year+`total_year_amt`] ?? 0) !== 0 ? (margin_last_ym_value - sga_last_ym_value) / flat_rsp[`_`+id_value+`_`+last_year+`total_year_amt`] : 0;
                    oTemp = {
                        type : org_chk,
                        rowType : "rohc",
                        seq : 9,
