module.exports = (srv) => {
    srv.on('get_pl_treemap_month_rate', async (req) => {
        ...
        const pl_where_conditions = { 'year': { in: [year] }, 'month': {'between': '01', 'and': month}, 'div_id': {'!=': null} };
...
let sga_query = [];
        const [query, query_target] = await Promise.all([
            // PL 실적, 목표 조회
            SELECT.from(pl_view).columns(pl_cols).where(pl_wheres).groupBy(...pl_groupBys).orderBy(...pl_orderBy),
                ...
                if(Array.isArray(data)){
                    data.forEach(data =>{
                        oResult.push({
                            ...
                            progress: data["sale_amount_sum"] / target_data["target_sale_amount"] * 100
                        })
                    });
                }else{
                    oResult.push({
                ...
                if(Array.isArray(data)){
                    data.forEach(data =>{
                        oResult.push({
                            ...
                            progress: data["margin_amount_sum"] / target_data["target_margin_amount"] * 100
                        })
                    });
                }else{
                    oResult.push({
