...
module.exports = (srv) => {
    srv.on('get_actual_rohc_account_oi', async (req) => {
        ...
        const pl_col_list = ['year', 'biz_tp_account_cd', s_margin_column];
        ...
        // rsp 조회용 정보
        const rsp_col_list = ['year', 'biz_tp_account_cd', 'sum(ifnull(total_year_amt,0)) as total_year_amt'];
        ...
        rsp_data.forEach(o_rsp=>{
            ...
            if(o_find_account){
                ...
                if(o_rsp.year = year){
                    o_data[`${o_rsp.biz_tp_account_cd}`]['curr_total_year_amt'] = (o_rsp?.total_year_amt ?? 0)
                }else{
                    o_data[`${o_rsp.biz_tp_account_cd}`]['last_total_year_amt'] = (o_rsp?.total_year_amt ?? 0)
                }
            }
        })
        ...
        a_data.forEach((o_rohc_data,i)=>{
            ...
            const o_temp =
            {
                ...
                "actual_curr_ym_value": (o_rohc_data?.curr_total_year_amt ?? 0) === 0 ? 0 : (o_rohc_data?.curr_margin ?? 0)/o_rohc_data.curr_total_year_amt,
                "actual_last_ym_value": (o_rohc_data?.last_total_year_amt ?? 0) === 0 ? 0 : (o_rohc_data?.last_margin ?? 0)/o_rohc_data.last_total_year_amt,
                "actual_curr_ym_rate": (o_rohc_data?.curr_target ?? 0) === 0 || (o_rohc_data?.curr_total_year_amt ?? 0) === 0 ? 0 : ((o_rohc_data?.curr_margin ?? 0)/o_rohc_data.curr_total_year_amt)/(o_rohc_data.curr_target*100000000),
                "actual_last_ym_rate": (o_rohc_data?.last_target ?? 0) === 0 || (o_rohc_data?.last_total_year_amt ?? 0) === 0 ? 0 : ((o_rohc_data?.last_margin ?? 0)/o_rohc_data.last_total_year_amt)/(o_rohc_data.last_target*100000000),
            };
