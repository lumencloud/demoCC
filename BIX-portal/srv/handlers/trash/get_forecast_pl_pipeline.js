...
module.exports = (srv) => {
    srv.on('get_forecast_pl_pipeline', async (req) => {
        ...
        let org_name = orgInfo[org_col_nm.split('_',1) + '_name'];
        ...
        if(type === 'month'){
            ...
            const pl_col_list = ['year', search_org, search_org_name, ...sale_sum_col, ...rodr_sum_col, ...rodr_count_sum_col];
            ...
            pl_data.forEach(data=>{
                ...
                if(!exist && data[search_org_name]){
                    org_list.push({id:data[search_org], name:data[search_org_name]});
                };
            });
            ...
            const pl_col_list = ['year', 'deal_stage_cd', search_org, search_org_name, 'sum(ifnull(sale_year_amt,0)) as sale_year_amt', 'sum(ifnull(rodr_year_amt,0)) as rodr_year_amt', ...rodr_count_sum_col, ...sale_sum_col, ...rodr_sum_col,];
            const pl_where_conditions = {'year': year, 'src_type':'D', 'deal_stage_cd': {'!=': null}};
            const pl_groupBy_cols = ['year', 'deal_stage_cd', search_org, search_org_name];
            ...
            const pl_col_list = ['year', search_org, search_org_name, ...rodr_col, ...sale_col];
            const pl_where_conditions = {'year': year, 'src_type':'D'};
