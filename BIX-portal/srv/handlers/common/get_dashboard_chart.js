module.exports = (srv) => {
    srv.on('get_dashboard_chart', async (req) => {
        try{
            ...
            let search_org, search_org_name, target_where;
            ...
            if(org_col_nm === 'lv1_id'){
                ...
                target_where = {'target_year': { in: [year, last_year] }};
            }else if(org_col_nm === 'lv2_id' || org_col_nm === 'lv3_id'){
                search_org = 'div_id';
                search_org_name = 'div_name';
                let aAddList = [search_org, search_org_name];
                pl_col_list.push(...aAddList);
                target_where =  {'target_year': { in: [year, last_year] }, [org_ccorg_col] : org_ccorg, 'div_ccorg_cd' : {'!=':null}, 'hdqt_ccorg_cd' : null };
            }else if(org_col_nm === 'div_id'){
                search_org = 'hdqt_id';
                search_org_name = 'hdqt_name';
                let aAddList = [search_org, search_org_name];
                pl_col_list.push(...aAddList);
                target_where =  {'target_year': { in: [year, last_year] }, 'org_ccorg_cd' : org_ccorg };
            ...
            const pl_groupBy_cols = ['year', 'org_order', search_org, search_org_name];
            ...
            let pl_sum_data = {
                ...
                "marginrate_curr_y_target": 0,//금년 마진율 목표
                "marginrate_curr_ym_amt": 0,//금년 1~마감월 마진율
                "marginrate_last_ym_amt": 0,//작년 1~마감월 마진율
                "marginrate_curr_ym_rate": 0, // ((금년 1~마감월 마진합산) / (금년 1~마감월 매출합산)) / 금년목표마진율 *100
