module.exports = (srv) => {
    srv.on('get_actual_m_oi', async (req) => {
        ...
        const rsp_col_list = ['month_amt', 'sum(ifnull(total_amt,0)) as total_amount', 'sum(ifnull(bill_amt,0)) as bill_amount', 'sum(ifnull(total_amt_sum,0)) as total_amount_sum', 'sum(ifnull(bill_amt_sum,0)) as bill_amount_sum'];
