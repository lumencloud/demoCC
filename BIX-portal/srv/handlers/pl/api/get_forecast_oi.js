module.exports = (srv) => {
    srv.on('get_forecast_oi', async (req) => {
        ...
        const pl_where_conditions = { 'year': { in: [year, last_year] }, 'month_amt': '12', 'is_delivery': true, 'src_type': { '!=':'WA'}};
        ...
        const sga_where_conditions = { 'year': { in: [year, last_year] }, 'month_amt': '12', 'is_delivery': true };
        const sga_groupBy_cols = ['year', 'shared_exp_yn'];

        // rsp 조회용 정보
        const rsp_col_list = ['year', 'sum(ifnull(total_amt_sum,0)) as total_amt_sum', 'sum(ifnull(bill_amt_sum,0)) as bill_amt_sum'];
        const rsp_where_conditions = { 'year': { in: [year, last_year] }, 'month_amt': '12', 'is_delivery': true };
        ...
        // DB 쿼리 실행 (병렬)
        const [pl_data, sga_data, rsp_data, dt_pl_data, target_data] = await Promise.all([
            // PL 실적, 목표 조회
        ...
        let a_curr_target = target_data.find(a=>a.year === year),
        ...
        dt_pl_data.forEach(a=>{
            if(a.year === year){
                if(a.src_type==='D'){
                    o_data['dt'][`${year}`]['not_secured'] = (o_data['dt'][`${year}`]?.['not_secured'] ?? 0) + a['sale_amount_sum'];
                }else{
                    o_data['dt'][`${year}`]['secured'] = (o_data['dt'][`${year}`]?.['secured'] ?? 0) + a['sale_amount_sum'];
                }
                o_data['dt'][`${year}`]['forecast_value'] = (o_data['dt'][`${year}`]?.['forecast_value'] ?? 0) + a['sale_amount_sum'];                
            }else{
                if(a.src_type==='D'){
                    o_data['dt'][`${last_year}`]['not_secured'] = (o_data['dt'][`${last_year}`]?.['not_secured'] ?? 0) + a['sale_amount_sum'];
                }else{
                    o_data['dt'][`${last_year}`]['secured'] = (o_data['dt'][`${last_year}`]?.['secured'] ?? 0) + a['sale_amount_sum'];
                }
                o_data['dt'][`${last_year}`]['forecast_value'] = (o_data['dt'][`${last_year}`]?.['forecast_value'] ?? 0) + a['sale_amount_sum'];
            }
        })

        let dt_curr_y_data = o_data['dt'][`${year}`];
        let dt_last_y_data = o_data['dt'][`${last_year}`];
        ...
        const dt_sale_data =
        {
            "display_order": 1,
            "type": "DT 매출",
            "forecast_value": dt_curr_y_data?.['forecast_value'] ?? 0,
            "secured_value": dt_curr_y_data?.['secured'] ?? 0,
            "not_secured_value": dt_curr_y_data?.['not_secured'] ?? 0,
            "plan_ratio": (dt_curr_y_data?.['forecast_value'] ?? 0) - ((a_curr_target?.['dt_sale_target'] ?? 0) * 100000000 ),
            "yoy": (dt_curr_y_data?.['forecast_value'] ?? 0) - ((a_last_target?.['dt_sale_target'] ?? 0) * 100000000 )
            // "plan_ratio": (dt_curr_y_data?.['forecast_value'] ?? 0) === 0 ? 0 : (dt_curr_y_data?.['secured'] ?? 0) / dt_curr_y_data['forecast_value'] * 100,
            // "yoy": (dt_curr_y_data?.['forecast_value'] ?? 0) === 0 || (dt_curr_y_data?.['secured'] ?? 0) === 0 || (dt_last_y_data?.['forecast_value'] ?? 0) === 0 ? 0 : ((dt_last_y_data?.['secured'] ?? 0) / dt_last_y_data['forecast_value'])/(dt_curr_y_data['secured'] / dt_curr_y_data['forecast_value']) * 100
        };
