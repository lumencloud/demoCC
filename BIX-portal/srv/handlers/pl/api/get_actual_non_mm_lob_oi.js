...
module.exports = (srv) => {
    srv.on('get_actual_non_mm_lob_oi', async (req) => {
        ...
        const target_col_list = [
            'year', 'sum(ifnull(non_mm_target,0)) as non_mm_target'];
         ...
         */
        const pl_col_list = [
            'year', 'bd_n2_cd', 'sum(ifnull(sale_amount_sum,0)) as sale_amount_sum', 'sum(ifnull(margin_amount_sum,0)) as margin_amount_sum'];
        const pl_where_conditions = { 'year': { in: [year, last_year] }, 'month_amt': month, 'is_delivery': true, 'src_type': { 'not in':['WA','D']}, 'bd_n2_cd': { '!=':null}};
        ...
        // DB 쿼리 실행 (병렬)
        const [target_data, pl_data, code_item_data] = await Promise.all([
        ...
        pl_data.forEach(o_item=>{
            ...
            if(o_find_code){
                if(!o_data[`${o_item.bd_n2_cd}`]){
                    o_data[`${o_item.bd_n2_cd}`]={lob_cd: o_item.bd_n2_cd, sort_order:o_find_code.sort_order};
                }
                if(o_item.year === year){
                    o_data[`${o_item.bd_n2_cd}`]['curr_sale'] = o_item?.sale_amount_sum ?? 0;
                    o_data[`${o_item.bd_n2_cd}`]['curr_margin'] = o_item?.margin_amount_sum ?? 0;
                    o_data[`${o_item.bd_n2_cd}`]['curr_margin_rate'] = (o_item?.sale_amount_sum ?? 0) === 0 ? 0 : o_item.margin_amount_sum/(o_item?.sale_amount_sum ?? 0)*100;
                }else{
                    o_data[`${o_item.bd_n2_cd}`]['last_sale'] = o_item?.sale_amount_sum ?? 0;
                    o_data[`${o_item.bd_n2_cd}`]['last_margin'] = o_item?.margin_amount_sum ?? 0;
                    o_data[`${o_item.bd_n2_cd}`]['last_margin_rate'] = (o_item?.sale_amount_sum ?? 0) === 0 ? 0 : o_item.margin_amount_sum/(o_item?.sale_amount_sum ?? 0)*100;
                }
            }
        })
        ...
        a_data.forEach(a_data=>{
            let o_sale_temp = {
                "display_order": ((a_data.sort_order+1)*3)-2,
                "lob_name": a_data.lob_cd,
                "type":'매출',
                "target_curr_y_value":  0,
                "actual_curr_ym_value":  a_data?.curr_sale ?? 0,
                "actual_last_ym_value":  a_data?.last_sale ?? 0,
                "actual_curr_ym_rate":  0,
                "actual_last_ym_rate":  0,
            }
            let o_margin_temp = {
                "display_order": ((a_data.sort_order+1)*3)-1,
                "lob_name": a_data.lob_cd,
                "type":'마진',
                "target_curr_y_value":  0,
                "actual_curr_ym_value":  a_data?.curr_margin ?? 0,
                "actual_last_ym_value":  a_data?.last_margin ?? 0,
                "actual_curr_ym_rate":  0,
                "actual_last_ym_rate":  0,
            }
            let o_margin_rate_temp = {
                "display_order": (a_data.sort_order+1)*3,
                "lob_name": a_data.lob_cd,
                "type":'마진율',
                "target_curr_y_value":  0,
                "actual_curr_ym_value":  a_data?.curr_margin_rate ?? 0,
                "actual_last_ym_value":  a_data?.last_margin_rate ?? 0,
                "actual_curr_ym_rate":  0,
                "actual_last_ym_rate":  0,
            }
