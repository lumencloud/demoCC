module.exports = (srv) => {
    srv.on('get_actual_dt_account_oi', async (req) => {
        ...
        const dt_pl_col_list = [
            'year', 'org_id', 'biz_tp_account_cd', 'sum(ifnull(sale_amount_sum,0)) as sale_amount_sum'];
        const dt_pl_where_conditions = { 'year': { in: [year, last_year] }, 'month_amt': month, 'is_delivery': true, 'src_type': { 'not in':['WO','D']},'dgtr_task_cd': {'!=':null}};
        const dt_pl_groupBy_cols = ['year', 'org_id', 'biz_tp_account_cd'];
        ...
        // DB 쿼리 실행 (병렬)
        const [dt_pl_data, account_data, target_data] = await Promise.all([
        ...
        dt_pl_data.forEach(o_item=>{
            ...
            if(o_find_account){
                ...
                if(o_item.year === year){
                    o_data[`${o_item.biz_tp_account_cd}`][`${year}`] = (o_data[`${o_item.biz_tp_account_cd}`][`${year}`] || 0 ) + o_item.sale_amount_sum;
                }else{
                    o_data[`${o_item.biz_tp_account_cd}`][`${last_year}`] = (o_data[`${o_item.biz_tp_account_cd}`][`${last_year}`] || 0 ) + o_item.sale_amount_sum;
                }
            }
        })
        ...
        a_result.forEach(o_data=>{
            let o_temp = {
                "display_order": o_data['sort_order']+1,
                "account_cd": o_data['account_cd'],
                "account_nm": o_data['account_nm'],
                "target_curr_y_value": 0,
                "actual_curr_ym_value": o_data?.[`${year}`] ?? 0,
                "actual_last_ym_value": o_data?.[`${last_year}`] ?? 0,
