...
module.exports = (srv) => {
    srv.on('get_actual_sale_account_pl', async (req) => {
        try{
            ...
            const account_cov_pl_view = db.entities('pl').account_convert_view;
            ...
            let sale_sum_total_col = `case when year='${last_year}' then (` + a_sale_total_column.join(' + ') + ') else 0 end as sale_total_amount_sum';
            ...
            const pl_where_conditions = {
                'year': { in: [year] }, 'src_type': { 'not in': ['WO', 'D'] },
                'biz_tp_account_cd': { '!=': null, and: { 'biz_tp_account_cd': { '!=': '' } } }, cstco_name: { '!=': null }
            };
            const pl_groupBy_cols = ['year', 'biz_tp_account_cd'];

            const account_cov_pl_col_list = ['year', 'biz_tp_account_cd', s_sale_column, sale_sum_total_col];
            const account_cov_pl_where_conditions = { 'year': { in: [last_year] }, 'biz_tp_account_cd': { '!=': null, and: { 'biz_tp_account_cd': { '!=': '' } } } };
            const account_cov_pl_groupBy_cols = ['year', 'biz_tp_account_cd'];
            ...
            let account_cov_pl_column = account_cov_pl_col_list;
            let account_cov_pl_where = org_col_nm === 'lv1_id' ? account_cov_pl_where_conditions : { ...account_cov_pl_where_conditions, [org_col_nm]: org_id };
            let account_cov_pl_groupBy = account_cov_pl_groupBy_cols;
            // DB 쿼리 실행 (병렬)
            const [target_data, pl_data, account_data, account_cov_pl_data] = await Promise.all([
                ...
                SELECT.from(account_cov_pl_view).columns(account_cov_pl_column).where(account_cov_pl_where).groupBy(...account_cov_pl_groupBy),
            ]);
            let chk_pl = pl_data.find(data=>data.sale_amount_sum);
            if(display_type !== 'chart' && !chk_pl){
            ...
            account_cov_pl_data.forEach(o_pl => {
                let o_find_account = account_data.find(o_account => o_account.biz_tp_account_cd === o_pl.biz_tp_account_cd);
                if (o_find_account) {
                    if (!o_data[`${o_pl.biz_tp_account_cd}`]) {
                        o_data[`${o_pl.biz_tp_account_cd}`] = { account_cd: o_pl.biz_tp_account_cd, account_nm: o_find_account.biz_tp_account_nm, sort_order: o_find_account.sort_order };
                    }
                    if (o_pl.year === last_year) {
            ...
            account_data.forEach(data =>{
                ...
                const sale_data =
                {
                    ...
                    "type": "매출",
                    "target_curr_y_value": (flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0),
