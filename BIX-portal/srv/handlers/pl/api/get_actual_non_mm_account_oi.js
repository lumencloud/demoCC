    ...
    srv.on('get_actual_non_mm_account_oi', async (req) => {
        try{
            ...
            const target_col_list = [
                ...
                'sum(margin_target) as margin_target'
            ];
            ...
            pl_data.forEach(o_pl => {
                ...
                // if (!o_pl.biz_tp_account_cd) {
                //     return;
                //     o_pl.biz_tp_account_cd = '-'
                //     o_find_account = { biz_tp_account_cd: '-', biz_tp_account_nm: '-', sort_order: 99 };
                // };
                if (o_find_account) {
            ...
            account_data.forEach((data, i) => {
                ...
                a_data.forEach(a_data => {
                    if (a_data.account_cd === data.biz_tp_account_cd) {
                        ...
                        // o_sale_temp["actual_curr_ym_rate"] = (flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0) === 0 ? 0 : (a_data?.curr_sale ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0) * 100000000);
                        // o_sale_temp["actual_last_ym_rate"] = (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_sale_target`] ?? 0) === 0 ? 0 : (a_data?.last_sale ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_sale_target`] ?? 0) * 100000000);

                        o_margin_temp["actual_curr_ym_value"] = a_data?.curr_margin ?? 0;
                        o_margin_temp["actual_last_ym_value"] = a_data?.last_margin ?? 0;
                        // o_margin_temp["actual_curr_ym_rate"] = (flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) === 0 ? 0 : (a_data?.curr_sale ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) * 100000000);
                        // o_margin_temp["actual_last_ym_rate"] = (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) === 0 ? 0 : (a_data?.last_sale ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) * 100000000);
                        ...
                        // o_margin_rate_temp["actual_curr_ym_rate"] = ((flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) === 0 ? 0 : (flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) / (flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0)) === 0 ? 0 : (flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) === 0 ? 0 : (a_data?.curr_margin_rate ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) / (flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0));
                        // o_margin_rate_temp["actual_last_ym_rate"] = ((flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) === 0 ? 0 : (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) / (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_sale_target`] ?? 0)) === 0 ? 0 : (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) === 0 ? 0 : (a_data?.last_margin_rate ?? 0) / ((flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_margin_target`] ?? 0) / (flat_target?.[`_${data.biz_tp_account_cd}_${last_year}_sale_target`] ?? 0));
                    };
                    ...
                    // o_margin_rate_temp["target_curr_y_value"] = (flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) === 0 ? 0 : (flat_target?.[`_${data.biz_tp_account_cd}_${year}_margin_target`] ?? 0) / (flat_target?.[`_${data.biz_tp_account_cd}_${year}_sale_target`] ?? 0);
                })
            ...
            // 조직 정보가 존재하지 않는 데이터
            // let null_data = pl_data.filter(oData => !oData.biz_tp_account_cd && oData.year === year);
            // if (null_data) {
            //     let o_sale_temp = {
            //         "display_order": 99,
            //         "item_order": 1,
            ...
            //         "account_nm": '기타',
            //         "type": '매출',
            //         "actual_curr_ym_value": null_data.reduce((iSum, oData) => iSum += parseInt(oData.sale_amount_sum), 0),
            //     }
            //     let o_margin_temp = {
            //         "display_order": 99,
            //         "item_order": 2,
            ...
            //         "account_nm": '기타',
            //         "type": '마진',
            //         "actual_curr_ym_value": null_data.reduce((iSum, oData) => iSum += parseInt(oData.margin_amount_sum), 0),
            //     }
            //     let o_margin_rate_temp = {
            //         "display_order": 99,
            //         "item_order": 3,
            ...
            //         "account_nm": '기타',
            //         "type": '마진율',
            //         "actual_curr_ym_value": (o_sale_temp.actual_curr_ym_value ?? 0) === 0 ? 0 : (o_margin_temp.actual_curr_ym_value / o_sale_temp.actual_curr_ym_value),
            //     }
            //     oResult.push(o_sale_temp, o_margin_temp, o_margin_rate_temp);
            // };
