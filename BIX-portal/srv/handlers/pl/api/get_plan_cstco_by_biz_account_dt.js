...
module.exports = (srv) => {
    srv.on('get_plan_cstco_by_biz_account_dt', async (req) => {
        try{
            /**
             * API 리턴값 담을 배열 선언
             */
            ...
            const pl_column = [
                'cstco_cd', 'cstco_name',
                `sum(case when src_type = 'D' then 0 else sale_year_amt end) as secured`,
                `sum(case when src_type = 'D' then ${s_forecast_sale} else 0 end) as not_secured`,
                `sum(case when src_type = 'D' then 0 else sale_year_amt end) 
                    + sum(case when src_type = 'D' then ${s_forecast_sale} else 0 end) as forecast`
            ];

            const pl_where = {
                year: year,
                dgtr_task_cd: { '!=': null, and: { dgtr_task_cd: { '!=': '' } } },
                src_type: { '!=': 'WA' },
                cstco_name: { '!=': null },
                [orgInfo.org_level]: orgInfo.org_ccorg_cd,
            ...
            let [pl_data] = await Promise.all([
                SELECT.from(pl_view).columns(pl_column).where(pl_where).groupBy(...pl_groupBy).orderBy("forecast desc").limit(5),
            ]);
            if(!pl_data.length){
