module.exports = (srv) => {
    srv.on('get_actual_rohc_org_oi', async (req) => {
         ...
         */
        const pl_col_list = ['year', 'sum(ifnull(margin_amount_sum,0)) as margin_amount_sum'];
        ...
        // DB 쿼리 실행 (병렬)
        const [target_data,pl_data,sga_data, rsp_data] = await Promise.all([
        ...
        pl_data.forEach(o_pl=>{
            if(!o_data[`${o_pl.id}`]){
                ...
                o_data[`${o_pl.id}`]={id:o_pl.id, name:o_pl.name,curr_target:(o_curr_target?.rohc_target ?? 0),last_target:(o_last_target?.rohc_target ?? 0)}
            }
            if(o_pl.year = year){
        ...
        rsp_data.forEach(o_rsp=>{
            if(!o_data[`${o_rsp.id}`]){
                ...
                o_data[`${o_rsp.id}`]={id:o_rsp.id, name:o_rsp.name,curr_target:(o_curr_target?.rohc_target ?? 0),last_target:(o_last_target?.rohc_target ?? 0)}
            }
            if(o_rsp.year = year){
        ...
        a_data.forEach((o_rohc_data,i)=>{
            ...
            {
                "display_order": i+2,
                "org_id": o_rohc_data.id,
                "org_nm": o_rohc_data.name,
                ...
                "actual_curr_ym_rate": (o_rohc_data?.curr_target ?? 0) === 0 || (o_rohc_data?.curr_total_amt_sum ?? 0) === 0 ? 0 : (((o_rohc_data?.curr_margin ?? 0)-(o_rohc_data?.curr_amount_sum ?? 0))/o_rohc_data.curr_total_amt_sum)/o_rohc_data.curr_target,
                "actual_last_ym_rate": (o_rohc_data?.last_target ?? 0) === 0 || (o_rohc_data?.last_total_amt_sum ?? 0) === 0 ? 0 : (((o_rohc_data?.last_margin ?? 0)-(o_rohc_data?.last_amount_sum ?? 0))/o_rohc_data.last_total_amt_sum)/o_rohc_data.last_target,
            };
            o_total['target_curr_y_value'] = (o_total['target_curr_y_value'] || 0) + o_temp['target_curr_y_value']
            o_total['target_last_y_value'] = (o_total['target_last_y_value'] || 0) + (o_rohc_data?.last_target ?? 0)
