        ...
        try{
            ...
            org_data.forEach(org => {
                ...
                o_result[`${org.ccorg_cd}`] = {
                    ...
                    org_name: org.name,
                    target_margin_rate : (o_target?.target_margin_rate ?? 0),     
                    target_margin : (o_target?.target_margin ?? 0)*100000000,     
                    target_sale : (o_target?.target_sale ?? 0)*100000000,     
                    curr_sale: i_curr_sale,                        // 올해 매출
                    curr_margin: i_curr_margin,                    // 올해 마진
                    last_sale: i_last_sale,                        // 작년 매출
                    last_margin: i_last_margin,                    // 작년 마진
                    margin_rate: i_curr_sale === 0 ? 0 : i_curr_margin / i_curr_sale * 100,  //마진율
                    crov_t_sale: i_crov_t_sale,                         // 신규 매출
                    crot_t_margin_rate: i_crov_t_sale === 0 ? 0 : i_crov_t_margin / i_crov_t_sale,     // 신규 마진율
                    crov_f_sale: i_crov_f_sale,                         // 이월 매출
                    crov_f_margin_rate: i_crov_f_sale === 0 ? 0 : i_crov_f_margin / i_crov_f_sale,     // 이월 마진율
                    relsco_t_sale: i_relsco_t_sale,                     // 대내 매출
                    relsco_t_margin_rate: i_relsco_t_sale === 0 ? 0 : i_relsco_t_margin / i_relsco_t_sale, // 대내마진율
                    relsco_f_sale: i_relsco_f_sale,                         // 대외 매출
                    relsco_f_margin_rate: i_relsco_f_sale === 0 ? 0 : i_relsco_f_margin / i_relsco_f_sale, //대외마진율
                    yoy_sale_rate: i_last_sale === 0 ? 0 : (i_curr_sale - i_last_sale) / i_last_sale ?? 0,   // 전년대비 매출 증감율
                    yoy_margin_rate: i_last_margin === 0 ? 0 : (i_curr_margin - i_last_margin) / i_last_margin ?? 0   // 전년대비 마진 증감율
                }
            })
