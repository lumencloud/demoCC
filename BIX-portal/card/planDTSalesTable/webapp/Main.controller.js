sap.ui.define([
    ...
    "../../main/util/Module",
], function (Controller, JSONModel, ODataModel, EventBus, Column, Label, Text, Modules, InteractionUtils, AgentService, AIPopupManager, HashChanger, Module) {
    "use strict";
    ...
    return Controller.extend("bix.card.planDTSalesTable.Main", {
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            if (oSelectData.find(oData => oData.sub_key === "org_delivery")) {  // Delivery 조직
                aBindingPath.push(`/get_forecast_dt_org_oi(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',org_tp='delivery')`);
            }

            if (oSelectData.find(oData => oData.sub_key === "org_account")) {   // Account 조직
                aBindingPath.push(`/get_forecast_dt_org_oi(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',org_tp='account')`);
            }

            aBindingPath.push(
                `/get_forecast_dt_account_oi(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}')`,
                `/get_forecast_dt_task_oi(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}')`,
                `/get_forecast_dt_customer_oi(year='${iYear}',org_id='${oData.orgId}')`,
                `/get_forecast_dt_task_year_oi(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}')`,
            );
