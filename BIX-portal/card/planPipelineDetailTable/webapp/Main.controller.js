sap.ui.define([
    ...
    return Controller.extend("bix.card.planPipelineDetailTable.Main", {
        ...
        _setData: async function(sChannelId, sEventId, oData){
            ...
            let sDealPath =`/get_forecast_pl_pipeline(year='${iYear}',month='${sMonth}',org_id='${sOrgId}',type='deal')`
            let sMonthPath = `/get_forecast_pl_pipeline(year='${iYear}',month='${sMonth}',org_id='${sOrgId}',type='month')`
            let sRodrPath = `/get_forecast_pl_pipeline(year='${iYear}',month='${sMonth}',org_id='${sOrgId}',type='rodr')`
    ...
    _monthVisibleSetting : function(aResults) {
        ...
        onAfterRendering: function () {
            let aTableList=["pipeDetailTable1", "pipeDetailTable2", "pipeDetailTable3"]
            aTableList.forEach(
                function(sTableId){
        ...
        _tableHeaderSetting : function (oTable, aEmphasisSetting = []) {
            ...
            for (const oColumn of aColumns) {
                let aMultiLabels = oColumn.getAggregation("label");
