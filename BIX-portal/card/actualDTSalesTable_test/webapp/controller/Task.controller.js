sap.ui.define([
    ...
    "use strict";
    return Controller.extend("bix.card.actualDTSalesTable_test.controller.Task", {
        /**
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            this.byId("actualDTSalesTaskBox").setBusy(true);
            ...
            oModel.bindContext(sTaskPath).requestObject().then(function (aResults) {
                this.getView().setModel(new JSONModel(aResults.value), "oTaskTableModel")
                // 테이블 로우 셋팅
                this._setVisibleRowCount(aResults);
                this.byId("actualDTSalesTaskBox").setBusy(false);
            }.bind(this)
            )
        },
        /**
        ...
        _setVisibleRowCount: function (aResults) {
            //테이블 리스트
            // 테이블 아이디로 테이블 객체
            let oTable = this.byId("actualDTSalesTaskTable")
