  ...
  function (BaseController, EventBus, ODataModel, JSONModel, NumberFormat, Module) {
    ...
    return BaseController.extend("bix.card.deselected.card", {
      ...
      _dataSetting: async function (oEvent, sEventId) {
        ...
        let sPath = oData ? `/ai_agent_view3(start_date='${oData.start_date}',end_date=${oData.end_date})/Set` :
          `/ai_agent_view3(start_date='${monday}',end_date=${sunday})/Set`
      ...
      _modelSetting: function (aResult) {
        ...
        let oModel = {
          iCount: iCount || 0,
          iAmount: this._formatAmountTotal(iAmount.toFixed()) || 0,
          first: aResult[0],
          second: aResult[1],
          third: aResult[2],
          forth: aResult[3],
      ...
      _formatTotal: function (sValue, sReason) {
        if (!sValue) return;
        ...
        if (Number.isInteger(fNumber)) {
          ...
          sValue = oNumberFormat.format(fTruncated);
        }
