sap.ui.define(
  ...
  function (BaseController, JSONModel, Modules, formatter, ODataModel, NumberFormat, EventBus) {
    "use strict";
    return BaseController.extend("bix.card.sgaProgress.card", {
      ...
      _setEventBusData: async function (oData) {
        ...
        for (let i = 3; i > 0; i--) {
          ...
          aLabels.push(`${sNewYear}-${sNewMonth}`);
      ...
      _setChart: async function () {
        ...
        oHTML.attachEventOnce("afterRendering", async function () {
          ...
          gradient.addColorStop(0, '#2196f3'); // 위쪽은 더 진하게
      ...
      //   // 차트 생성
      //   const canvas = document.getElementById('sgaProgress');
      //   const ctx = canvas.getContext('2d');
      ...
      //     });
      //   } else {
      //     this.oChartInstance.data.labels = aLabels;
      //     this.oChartInstance.data.datasets[0].data = aChartData;
      //     this.oChartInstance.options.scales.y.min = Math.min(...aChartData) * 0.8;
      //     this.oChartInstance.options.scales.y.max = Math.max(...aChartData) * 1.2;
      //     this.oChartInstance.update();
      //   }
      // },
    });
  }
