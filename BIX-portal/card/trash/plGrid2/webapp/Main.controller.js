...
sap.ui.define([
    ...
    return Controller.extend("bix.card.plGrid2.Main", {
        ...
        _setChartData: function (aResults, aLabels) {
            ...
            let aChartData2 = aResults.map(oResult => oResult.value.find(oData => oData.display_order == "2").actual_curr_ym_value);
            // let aChartData3 = aResults.map(oResult => oResult.value.find(oData => oData.seq == "4").performanceCurrentYearMonth);
            // let aChartData4 = aResults.map(oResult => oResult.value.find(oData => oData.seq == "5").performanceCurrentYearMonth);
            ...
            if(!isNaN(aChartData2[aChartData2.length-1]) && !isNaN(aChartData2[aChartData2.length-2])){
                let iChk = aChartData2[aChartData2.length-1] - aChartData2[aChartData2.length-2];
                let oIcon = this.byId("plGrid2ChartIcon");
                if(iChk > 0){
