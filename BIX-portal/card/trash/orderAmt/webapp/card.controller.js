  ...
  function (BaseController, formatter, EventBus, ODataModel, JSONModel, NumberFormat) {
    ...
    return BaseController.extend("bix.card.marginAmt.card", {
      ...
      _dataSetting: async function () {
        ...
        this._setUi(aResult.value[0].rodr_curr_ym_amt_rate, 1);
        this._setUi(aResult.value[0].rodr_curr_ym_cnt_rate, 2);

        this.getView().setBusy(false);
      },

      _setUi: function (iChk, i) {

        let oHBox = this.byId("icon_box" + i)
        let oIcon = this.byId("Icon" + i);
        let oIconText = this.byId("Icon_text" + i);
        if (iChk > 0) {
