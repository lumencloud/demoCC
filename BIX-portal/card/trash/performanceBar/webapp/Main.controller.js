		...
		_chartSetting1:function(){
			...
			let popoverProps = {
                'customDataControl' : function(data){
					...
					let oRate = aData.find(data => data.type === "마진률");
                    if(data){
                        let divStr = "";
						if(data.data.val[0].value === '마진률'){
							divStr = divStr + "<div style = 'margin: 15px 30px 0 30px'>" + "분류:" + "<span style = 'float: right'>" + data.data.val[0].value + "</b></div>";
							divStr = divStr + "<div style = 'margin: 5px 30px 0 30px'>" + "마진률:" + "<span style = 'float: right'>" + this.percentFormat(oRate.oldRate) + "</span></div>";
