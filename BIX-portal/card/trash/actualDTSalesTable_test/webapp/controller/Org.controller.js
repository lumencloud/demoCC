sap.ui.define([
    ...
    "use strict";
    return Controller.extend("bix.card.actualDTSalesTable_test.controller.Org", {
        /**
        ...
        _bindTable: async function (sChannelId, sEventId, oData) {
            ...
            this.byId("actualDTSalesOrgBox").setBusy(true);
            ...
            oModel.bindContext(sOrgPath).requestObject()
                .then(function (aResults) {
                    this.getView().setModel(new JSONModel(aResults.value), "oOrgTableModel")
                    // 테이블 로우 셋팅
                    this._setVisibleRowCount(aResults);
                    this.byId("actualDTSalesOrgBox").setBusy(false);
                }.bind(this)
                )
        },
        /**
        ...
        _setVisibleRowCount: function (aResults) {
            //테이블 리스트
            // 테이블 아이디로 테이블 객체
            let oTable = this.byId("actualDTSalesOrgTable")
