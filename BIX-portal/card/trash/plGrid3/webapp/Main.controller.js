...
sap.ui.define([
    ...
    return Controller.extend("bix.card.plGrid3.Main", {
        ...
        _setChartData: function (aResults, aLabels) {
            ...
            let aChartData3 = aResults.map(oResult => oResult.value.find(oData => oData.display_order == "4").actual_curr_ym_value);
            ...
            if(!isNaN(aChartData3[aChartData3.length-1]) && !isNaN(aChartData3[aChartData3.length-2])){
                let iChk = aChartData3[aChartData3.length-1] - aChartData3[aChartData3.length-2];
                let oIcon = this.byId("plGrid3ChartIcon");
                if(iChk > 0){
