sap.ui.define(
  ...
  function (BaseController, EventBus, ODataModel, JSONModel, NumberFormat, Module) {
    "use strict";
    return BaseController.extend("bix.card.monthlyCompletedContract.card", {
      _oEventBus: EventBus.getInstance(),

      onInit: function () {
        // this._dataSetting();
        this._oEventBus.subscribe("aireport", "contract", this._modelSetting, this);
      ...
      _dataSetting: async function () {
        ...
        // 전사 api
        let sAllPath = `/get_ai_forecast_deal_type_pl(year='${iYear}',month='${sMonth}',org_id='${sOrgId}',deal_stage_cd='contract')`
        // Account api
        let sAccountPath = `/get_ai_forecast_deal_type_pl(year='${iYear}',month='${sMonth}',org_id='${sOrgId}',deal_stage_cd='contract',org_tp='${oData.type}')`
      ...
      _modelSetting: function (sChannel, sEventId, oData) {
        ...
        this._oEventBus.publish("aiReport", "comMonthData", oModel)
