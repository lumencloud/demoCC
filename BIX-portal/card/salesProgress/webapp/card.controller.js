sap.ui.define(
  ...
  function (BaseController, JSONModel, Modules, formatter, ODataModel, NumberFormat, EventBus) {
    "use strict";
    return BaseController.extend("bix.card.salesProgress.card", {
      ...
      _setChart: async function () {
        ...
        oHTML.attachEvent("afterRendering", async function () {
          ...
          this._ovserveResize(this.byId(this._sContainerId))
      ...
      //   // 차트 생성
      //   const canvas = document.getElementById('salesProgress');
      //   const ctx = canvas.getContext('2d');
      ...
      //         datasets: [{
      //           data: [2050329178142, 1850329178142, 1550329178142],
      //           backgroundColor: gradient,
      //           fill: true,
      //           borderColor: "#2196f3",
      //           tension: 0.4,
      ...
      // let aResults = await Promise.all(aRequests.map(fn => fn()));
      // let aChartData = aResults.map(oResult => {
      //   return Math.floor(oResult.value.find(oData => oData.seq == "1").performanceAttainmentRateCurrentYear)
      // });
