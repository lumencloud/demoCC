sap.ui.define(
  ...
  function (BaseController, JSONModel, Modules, formatter, ODataModel, NumberFormat, EventBus) {
    "use strict";
    return BaseController.extend("bix.card.marginProgress.card", {
      ...
      _setChartData: async function (aRequests, aLabels) {
        ...
        let aProgress = aResults.map(oResult => {
          let iRes = 0;
          iRes = oResult.value.find(oData => oData.display_order == "1")?.target_curr_y_value ? Math.floor(oResult.value.find(oData => oData.display_order == "1").actual_curr_ym_value) / Math.floor(oResult.value.find(oData => oData.display_order == "1")?.target_curr_y_value * 100000000) * 100 : 0;
      ...
      _ovserveResize: function(oElement){
      ...
      //   let aResults = await Promise.all(aRequests.map(fn => fn()));
      //   let aChartData = aResults.map(oResult => {
      //     return Math.floor(oResult.value.find(oData => oData.seq == "1").performanceCurrentYearMonth)
      //   });
      ...
      //   // 차트 생성
      //   const canvas = document.getElementById('marginProgress');
      //   const ctx = canvas.getContext('2d');
      ...
      //   //   let sPath = `/get_pl_performance(year='${sYear}',month='${sMonth}',org_id='${sOrgId}')`;
      ...
      //   // let aResults = await Promise.all(aRequests.map(fn => fn()));
      //   // let aChartData = aResults.map(oResult => {
      //   //   return Math.floor(oResult.value.find(oData => oData.seq == "2").performanceAttainmentRateCurrentYear)
      //   // });
