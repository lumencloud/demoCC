    ...
    return Controller.extend("bix.card.actualSaleMarginChart.card", {
        ...
        _dataRequest: async function (sChannelId, sEventId, oData) {
            ...
            if (oSelectData.find(oData => oData.sub_key === "org")) {   // 조직
                aBindingPath["org"]=(`/get_actual_sale_org_pl(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',display_type='chart')`);
            }

            if (oSelectData.find(oData => oData.sub_key === "org_delivery")) {  // Delivery 조직
                aBindingPath["org_delivery"]=(`/get_actual_sale_org_pl(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',org_tp='delivery',display_type='chart')`);
            }
            if (oSelectData.find(oData => oData.sub_key === "org_account")) {   // Account 조직
                aBindingPath["org_account"] = (`/get_actual_sale_org_pl(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}',org_tp='account',display_type='chart')`);
            }

            aBindingPath["account"] = `/get_actual_sale_account_pl(year='${iYear}',month='${sMonth}',org_id='${oData.orgId}')`
            ...
            .then(function (aResults) {
                ...
                aList.forEach(
                    (sName)=>{
                    if(sName === "org"){
                        let oModel = this._chartSetting_type(aResults[i].value)
                        ...
                        let oModel = this._chartSetting_account(aResults[i].value)
                        this.getView().setModel(new JSONModel(oModel), "accountModel")
                        i++
                    }
                })
            }.bind(this))
        ...
        _chartSetting_type: function (aData) {
            ...
            let aMarginRate = aData.filter(data => data.type === '마진율')
            ...
            for (let i = 0; i < aSale.length; i ++) {
                ...
                let oModel = {
                    ...
                    "target_margin": oMargin.target_curr_y_value * 100000000,
                    "target_sale": aSale[i].target_curr_y_value * 100000000,
                    "curr_margin": oMargin.actual_curr_ym_value,
                    "curr_sale": aSale[i].actual_curr_ym_value,
                    "yoy_margin_rate": (oMargin.actual_last_ym_value) === 0 ? 0 : oMargin.actual_curr_ym_value_gap / (oMargin.actual_last_ym_value) * 100,
                    "yoy_sale_rate": (aSale[i].actual_last_ym_value) === 0 ? 0 : aSale[i].actual_curr_ym_value_gap / (aSale[i].actual_last_ym_value) * 100,
                    "last_sale": aSale[i].actual_last_ym_value,
                    "last_margin": oMargin.actual_last_ym_value,
                    "target_margin_rate": aMarginRate.target_curr_y_value,
                }
        ...
        _chartSetting_account: function (aData) {
            ...
            for (let i = 0; i < aData.length; i++) {
                let oModel = {
                    ...
                    "target_margin": aData[i].target_curr_y_value * 100000000,
                    "target_margin_rate": aData[i].target_curr_y_value * 100,
                    "target_sale": aData[i].target_curr_y_value * 100000000,
                    "curr_margin": aData[i].actual_curr_ym_value,
                    "curr_sale": aData[i].actual_curr_ym_value,
                    "yoy_margin_rate": aData[i].actual_last_ym_value === 0 ? 0 : aData[i].actual_curr_ym_value_gap / aData[i].actual_last_ym_value * 100,
                    ...
                    "last_margin": aData[i].actual_last_ym_value,
                }
                aModelData.push(oModel)
            }
            ...
            aModelData.forEach(
                function (data) {
                    ...
                    let oModel = {
                        ...
                        "margin_difference": this.onFormatPerformance(margin_difference, 'billion'),
                        "marginRate_target": this.onFormatPerformance(data.target_sale === 0 ? 0 : data.target_margin / data.target_sale * 100, 'percent2'),
                        "marginRate": this.onFormatPerformance(data.curr_sale === 0 ? 0 : data.curr_margin / data.curr_sale * 100, 'percent3'),
                        "marginRate_type": (data.curr_margin / data.curr_sale) >= 0 ? true : false,
                        "circleRate": this.onFormatPerformance(circleRate, 'percent2'),
