...
sap.ui.define([
    ...
    return Controller.extend("bix.card.plGrid5.Main", {
        ...
        _setChartData: function (aResults, aLabels) {
            ...
            let aChartData5 = aResults.map(oResult => oResult.value.find(oData => oData.display_order == "6").actual_curr_ym_value);
            ...
            if(!isNaN(aChartData5[aChartData5.length-1]) && !isNaN(aChartData5[aChartData5.length-2])){
                let iChk = aChartData5[aChartData5.length-1] - aChartData5[aChartData5.length-2];
                let oIcon = this.byId("plGrid5ChartIcon");
                if(iChk > 0){
