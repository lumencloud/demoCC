...
async function qa_setting() {
    ...
    const settingLibary = async function (fileList) {
        for (const library_path of fileList) {
            if (!library_path.includes("/_sample/")) {
                const orgin_path = library_path + "_origin";
                fs.renameSync(library_path, orgin_path);
                const library = fs.readFileSync(orgin_path, 'utf-8');
                let library_content = library.replace('bix.','qa.bix.');
                fs.writeFileSync(library_path, library_content, 'utf-8');
            }
        }
    };
    ...
    const uiLibaryList = await fg(['app/**/.library', '!**/dist/**']);
    settingLibary(uiLibaryList);
