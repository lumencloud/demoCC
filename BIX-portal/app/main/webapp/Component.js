...
], (UIComponent, models, ControlBehavior, AnimationMode, Localization, JSONModel, ODataModel) => {
    "use strict";
    ...
    return UIComponent.extend("bix.main.Component", {
        ...
        init: function () {
            ...
            this.getRouter().attachTitleChanged(function (oEvent) {
                ...
                document.title = 'BIX Portal' + ' - ' + sTitle; // oBundle.getText("p_t_portal")
                if (window.location.host.includes("-qa-")) document.title = "[QA] " + document.title;
                if (!window.location.host.includes("-qa-") && window.location.host.includes("-dev")) document.title = "[DEV] " + document.title;
                
                if (window.location.host.includes("port500")) document.title = "[LOCAL] " + document.title;
        ...
        getUserInfo() {
            const sUrl = "/odata/v4/cm/get_user_org_info"
            const oSetting = {
                type: "get",
                async: false,
                url: sUrl,
        ...
        _setManifestInfo(oMenu) {
            ...
            if (oMenu.init_yn === true) {
                sap.ui.core.Component.load({
                    name: sNestedCompName,
                    manifest: true
                })
            }
