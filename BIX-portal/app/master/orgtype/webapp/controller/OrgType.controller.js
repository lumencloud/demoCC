sap.ui.define([
  ...
  return Controller.extend("bix.master.orgtype.controller.App", {
    ...
    onChange: async function (oEvent) {
      ...
      let oChangeObject = oEvent.getSource().getBindingContext("orgModel").getObject(); // 변경시의 최종 데이터 가져옴

      let oModel = this.getOwnerComponent().getModel(); // 데이터 요청용 모델 바인딩
      let sBindingPathBase = "/OrgType" // 기본 path
      let sBindingPath = sBindingPathBase + "('" + oChangeObject.ccorg_cd + "')" //update용 path
      let aOrgTypeData = await this._getData(sBindingPathBase) // 데이터 새로 요청      

      let bFlag = aOrgTypeData.some(oData => oData.ccorg_cd === oChangeObject.ccorg_cd)
      ...
      if (bFlag) { //데이터 존재시 update, 데이터 없을시 create
        ...
        oContext.getBoundContext().setProperty("org_tp", oChangeObject.org_tp); // 변경데이터를 context로 저장
        oContext.getBoundContext().setProperty("is_delivery", oChangeObject.is_delivery); // 변경데이터를 context로 저장
        oContext.getBoundContext().setProperty("is_total_cc", oChangeObject.is_total_cc); // 변경데이터를 context로 저장
        oUiModel.hasUpdatingChanges = oModel.hasPendingChanges("update") // uiModel의 update 관련 속성을 false -> true로 변경
      } else {
        // create
        if (oChangeObject.is_delivery === null) { oChangeObject.is_delivery = false } // notNull 속성 지정되있으므로, null 일경우 false로 지정
        ...
        oBinding.create({
          ccorg_cd: oChangeObject.ccorg_cd, // key값 
          org_tp: oChangeObject.org_tp, // 입력된 데이터 
          is_delivery: oChangeObject.is_delivery, // 입력된 데이터 
          is_total_cc: oChangeObject.is_total_cc,
        })
