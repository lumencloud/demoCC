...
function manipulationTarget( elem, content ) {
	if ( nodeName( elem, "table" ) &&
		...
		let tbody = jQuery( elem ).children( "tbody" )[ 0 ];
		if (tbody) {
			return tbody;
		} else {
...
if (/(?:\?|&)sap-ui-xx-self-closing-check=(?:x|X|true)/.exec(window.location.search)) {
	let rNonVoidHtml5Tags = new RegExp(
	...
	jQuery.htmlPrefilter = function(html) {
		html.replace(rxhtmlTag, function($0, $1, $2) {
			if ( rNonVoidHtml5Tags.test($2) && $0.length < html.length ) {
				let replacement = "<" + $1 + "></" + $2 + ">";
				let noteUrl = "https://launchpad.support.sap.com/#/notes/2944336";
				let gitHubUrl = "https://github.com/SAP/openui5/blob/master/docs/self_closing_tags_fix_instructions.md";
				let errorMessage = "jQuery incompatibility: non-void HTML tags must not use self-closing syntax.\n"
				...
				let errorMessageWithUrl = errorMessage + noteUrl + " or\n" + gitHubUrl;
				...
				try {
					sap.ui.require(["sap/m/MessageBox", "sap/m/FormattedText", "sap/base/security/encodeXML"], function (MessageBox, FormattedText, encodeXML) {
						let messageText = new FormattedText({
