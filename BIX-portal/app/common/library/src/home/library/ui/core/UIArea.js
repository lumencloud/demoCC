...
sap.ui.define([
	...
	let oRenderLog = Rendering.getLogger();

	// create the RenderManager so it can be used already
	let oRenderManager = new RenderManager();
	...
	let fnDbgWrap = function(oControl) {
		return oControl;
	},
	...
	if ( oRenderLog.isLoggable() ) {
		...
		//  this supportability feature could be moved out of the standard runtime code and only be loaded on demand
	 ...
	 * @hideconstructor
	 */
	let UIArea = ManagedObject.extend("sap.ui.core.UIArea", {
	...
	UIArea.prototype.addContent = function(oContent, _bSuppressInvalidate) {
		...
		//  this remains here just to make the UX3 Shell work which doesn't invalidate properly
	...
	UIArea.prototype.removeContent = function(vContent, /* internal only */ _bSuppressInvalidate) {
		let oContent = this.removeAggregation("content", vContent, _bSuppressInvalidate);
	...
	UIArea.prototype.removeAllContent = function() {
		let aContent = this.removeAllAggregation("content");
		for (let idx = 0; idx < aContent.length; idx++) {
			let oDomRef;
			let oContent = aContent[idx];
			if (oContent && oContent.getDomRef) {
				oDomRef = oContent.getDomRef();
			}
			this._addRemovedContent(oDomRef);
	...
	UIArea.prototype.resumeInvalidationFor = function (oControl) {
		...
		let sId = oControl.getId();
		let mControlsSuppressedFromInvalidation = this.mSuppressedControls[sId];
		...
		mControlsSuppressedFromInvalidation.forEach(function(sControlId) {
			let oControl = Element.getElementById(sControlId);
			if (oControl) {
	...
	UIArea.prototype.addInvalidatedControl = function(oControl){
		...
		if ( this.iSuppressedControlsLength ) {
			for (let oCurrent = oControl; oCurrent; oCurrent = oCurrent.getParent()) {
				let mControlsSuppressedFromInvalidation = this.mSuppressedControls[oCurrent.getId()];
				if (mControlsSuppressedFromInvalidation) {
	 ...
	 */
	 UIArea.prototype.rerender = function(bForce) {
		...
		let bRenderSelf = this.bRenderSelf,
		...
		if (bRenderSelf) { // full UIArea rendering
			...
			if (this.oRootNode) {
				...
				let cleanUpDom = function(aCtnt, bCtrls){
					let len = aCtnt.length;
					let oDomRef;
					for (let i = 0; i < len; i++) {
				...
				let oFocusRef_Initial = document.activeElement;
				let oStoredFocusInfo = FocusHandler.getControlFocusInfo();

				//First remove the old Dom nodes and then render the controls again
				cleanUpDom(aContentToRemove);

				let aContent = this.getContent();
				let len = cleanUpDom(aContent, true);

				let oFocusRef_AfterCleanup = document.activeElement;
			...
			let isRenderedTogetherWithAncestor = function(oCandidate) {
			...
			let aControlsRenderedTogetherWithAncestor = [];
			for (let n in mInvalidatedControls) {
				let oControl = Element.getElementById(n);
	...
	UIArea.rerenderControl = function(oControl){
		...
		let oParentDomRef = oDomRef && oDomRef.parentNode; // remember parent here as preserveContent() might move the node!
		if (oParentDomRef) {
			let oUIArea = oControl.getUIArea();
			// Why this is really needed?
			let oRM = oUIArea ? oRenderManager : new RenderManager();
	...
	let rEvents = /^(mousedown|mouseup|click|keydown|keyup|keypress|touchstart|touchend|tap)$/;
	let aPreprocessors = [], aPostprocessors = [];
	let mVerboseEvents = {mousemove: 1, mouseover: 1, mouseout: 1, scroll: 1, dragover: 1, dragenter: 1, dragleave: 1};
	...
	UIArea.prototype._handleEvent = function(/**event*/oEvent) {
		...
		//this should be the 'lowest' SAPUI5 Control of this very
		...
		let sHandledUIAreaId = oEvent.getMark("handledByUIArea"),
			sId = this.getId();
		...
		while (oElement instanceof Element && oElement.isActive() && !oEvent.isPropagationStopped()) {
			let sScopeCheckId = oEvent.getMark("scopeCheckId"),
			...
			if (!oScopeCheckDOM || (oDomRef && oDomRef.contains(oScopeCheckDOM))) {
				...
				for (let i = 0, is = aEventTypes.length; i < is; i++) {
					let sType = aEventTypes[i];
					oEvent.type = sType;
	...
	*/
	...
	UIArea.prototype._handleGroupChange = function(oEvent, oElement) {
		let oKey = UIArea._oFieldGroupValidationKey;
		if (oEvent.type === "focusin" || oEvent.type === "focusout") {
			...
			let oCurrentControl = this.getFieldGroupControl(),
	...
	UIArea.prototype.setFieldGroupControl = function(oElement) {
		...
		if ( oControl != oCurrentControl && document.activeElement && (document.activeElement.id !== "sap-ui-static-firstfe")) {
			let aCurrentGroupIds = (oCurrentControl ? oCurrentControl._getFieldGroupIds() : []),
	...
	UIArea.create = function(vDomRef) {
		...
		let sId = oDomRef.id;
		let oUIArea = UIAreaRegistry.get(sId);
		if (!oUIArea) {
	...
	UIArea.setCore = function(oCoreInstance) {
		...
		if ( aUiAreas ) {
			for (let i = 0, l = aUiAreas.length; i < l; i++) {
