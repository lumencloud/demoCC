...
sap.ui.define([
    ...
    let oContentProviderLabelPromises = {};
    ...
    readUtils.getCdmParts = function (oSite, oTile) {
        let oViz = readVisualizations.get(oSite, readHome.getTileVizId(oTile)) || {};
        let oVizConfig = readVisualizations.getConfig(oViz);
        let oApp = readVisualizations.getAppDescriptor(oSite, readVisualizations.getAppId(oViz));
        let oInbound = readApplications.getInbound(oApp, readVisualizations.getInboundId(oViz));
    ...
    readUtils.harmonizeTarget = function (oTarget) {
        let vParameters = oTarget.parameters;
        let oParametersOut = {};
    ...
    readUtils._evaluateDisplayFormat = function (oVizData, oSite) {
        let sDisplayFormatHint = oVizData.displayFormatHint;
        let oVizType = readVisualizations.getType(oSite, oVizData.vizType);
        let sTileSize = readVisualizations.getTileSize(oVizType);

        // Set standard fallback displayFormat to StandardWide for 1x2 tiles
        let sStandardDisplayFormat = sTileSize === "1x2" ? DisplayFormat.StandardWide : DisplayFormat.Standard;
        let aDefaultDisplayFormats = [sStandardDisplayFormat, DisplayFormat.Compact];
        ...
        let sDefaultDisplayFormat = readVisualizations.getDefaultDisplayFormat(oVizType) || sStandardDisplayFormat;
    ...
    readUtils._addBookmarkInstantiationData = function (oVizData, oSite) {
        if (oVizData.vizType) {
            let oVizType = readVisualizations.getType(oSite, oVizData.vizType);
            let sVizTypeType = ObjectPath.get(["sap.app", "type"], oVizType || {});
            ...
            if (oVizType && sVizTypeType !== "chipVizType") {
                ...
                let oChipConfig = readVisualizations.getChipConfigFromVizReference(oVizData);
                oVizData._instantiationData = {
                    platform: "ABAP",
    ...
    readUtils.getBookmarkVizTypeIds = function (oBookmark) {
        ...
        let sCdmVizType = oBookmark.vizType;
        if (sCdmVizType && !aIds.includes(sCdmVizType)) {
            aIds.push(sCdmVizType);
        }

        let oChipConfig = readVisualizations.getChipConfigFromVizReference(oBookmark);
        let sChipId = oChipConfig && oChipConfig.chipId;
