sap.ui.define([
    ...
    var oExtendControl = {
        ...
        _handleDownload: function () {
            ...
            try {
                const oHighlight = Fragment.byId(this.getId(), "highlightBox") ||
                    Fragment.byId(this.getId(), "highlightText");
        ...
        _updateHighlightArea: function () {
            let oHighlightText = Fragment.byId(this.getId(), "highlightText");
            ...
            const htmlContent = this.getIsLoading() ?
                ...
                this._convertToHtmlWithStyle(this.getAiContent());


            oHighlightText.setContent(htmlContent);
        ...
        // 기존 _convertToHtml는 놔두고 style 적용한 함수로 따로 빼놓았습니다.
        // 추후 적용시 기존 _convertToHtml는 없애고 스타일적용된 함수를 _convertToHtml 명칭으로 쓰셔도 됩니다
        _convertToHtmlWithStyle: function (sText) {
            if (!sText) {
                return "<ul><li>AI 분석 결과가 없습니다.</li></ul>";
            }

            let aLines = sText.split('\n');
            let sHtml = '<dl class="list_type">';
            let bInSubList = false;
            ...
            aLines.forEach(line => {
                ...
                if (line.startsWith('•')) {
                    if (bInSubList) {
                        sHtml += '</dl><dl class="list_type">';
                        bInSubList = false;
                    }
                    sHtml += `<dt>${line.substring(1).trim()}</dt>`;
                }
                else if (line.startsWith('-')) {
                    if (!bInSubList) {
                        bInSubList = true;
                    }
                    sHtml += `<dd>${line.substring(1).trim()}</dd>`;
                }
            });
