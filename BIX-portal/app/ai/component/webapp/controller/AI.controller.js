    ...
    return BaseController.extend("bix.ai.component.controller.AI", {
        ...
        onAiTilePress: function (oEvent) {
            if (oEvent) {
                ...
                this.getView().setModel(new JSONModel({tileSelectTitle: this.byId("initTile").getHeader(), initPage:true}),"ui");
            }
        },
        /**
         * 미리 정의된 버튼 클릭 이벤트
        ...
        formatAgentName: function(agentId) {
            var agentMapping = {
                ...
                "report_content_agent": "리포트 컨텐츠 에이전트"
            };
        ...
        formatAgentIcon: function(agentId) {
            var iconMapping = {
                ...
                "report_content_agent": "./resource/icon/ai_agent/report.png"
            };
        ...
        _addAIMessage: function(result) {
            ...
            if (sSelectedAgent === "navigator_agent" && result.agent_result?.executive_summary) {
                ...
                // 네비게이터 처리 결과가 null이면 백엔드 메시지 그대로 사용
                if (oNavigationData && oNavigationData.message) {
                    sContent = oNavigationData.message;
                }
                // null인 경우 sContent는 백엔드에서 온 executive_summary 그대로 사용
            }
            ...
            // 포맷형식 변형에 따른 에러방지
            try {
                oAIMessage["content"] = this._contentFormat(sContent);
            } catch {
                oAIMessage["content"] = sContent;
            }
        ...
        // JSON형식 컨텐츠를 HTML형식을 변환
        _contentFormat: function (sContent) {
            let oFormatContent = "";
            if ((() => {try{ JSON.parse(sContent); return true} catch { return false; }})()) {
                ...
                if (oContent.insights) {
                    let sListItems = "";
                    oContent.insights.forEach((sItem) => {
                        sListItems += `<dd>${sItem}</dd>`
                    })
            ...
            oFormatContent = `<div class="bg_area"><dl class="list_type">${oFormatContent}</dl></div>`
            return oFormatContent;
        },
        ...
        _processNavigatorResult: function(executiveSummary) {
            try {
                ...
                var aSelectedMenus = oResult.map(function(menu) {
                    return {
                        id: menu.ID || menu.id,
                ...
                return null; // null 반환하여 백엔드 메시지 사용
            }
        },
