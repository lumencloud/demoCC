 ...
 */
entity Report {
    key id                      : String(50)    @title: '보고서ID'          @description: '보고서 고유 식별자';
        name                    : String(128)   @title: '보고서명'          @description: '보고서 이름';
        type                    : String(1)     @title: '타입'              @description: '보고서 타입(W:주간, M:월간, T:테스트)';
        target_org_id           : String(50)    @title: '대상조직ID'        @description: '보고서 대상 조직 ID';
        description             : String(1000)  @title: '보고서설명'        @description: '보고서의 기능과 용도에 대한 설명';
        trigger_patterns        : String(1000)  @title: '트리거패턴'        @description: '보고서 키워드 정보';
        route_template          : String(500)   @title: '라우트템플릿'      @description: 'URL 경로의 템플릿 패턴';
        file_path               : String(500)   @title: '파일경로'          @description: '보고서 파일 경로';
        created_dt              : Date          @title: '등록일자'          @description: '보고서 등록일';
        created_by              : String(50)    @title: '등록자'            @description: '보고서 등록자';

    // 관계: 해당 보고서의 컨텐츠들
    contents : Composition of many Report_Content on contents.report_id = $self.id;
}
...
entity Report_Content {
    key report_id               : String(50)    @title: '보고서ID'          @description: '보고서 고유 식별자';
    key id                      : String(50)    @title: '컨텐츠ID'          @description: '컨텐츠 식별자';
        sequence                : Integer       @title: '컨텐츠순번'        @description: '컨텐츠 항목 순서';
        name                    : String(128)   @title: '컨텐츠명'          @description: '컨텐츠 이름';
        type                    : String(20)    @title: '타입'              @description: '컨텐츠 타입(text, chart, grid, image)';
        content                 : String(2000)  @title: '컨텐츠내용'        @description: '컨텐츠 내용(JSON 포맷)';
        target_function         : String(200)   @title: '대상함수'          @description: '컨텐츠 구성 대상함수';
        created_dt              : Date          @title: '생성일자'          @description: '컨텐츠 생성일자';
        created_by              : String(50)    @title: '생성자'            @description: '컨텐츠 생성자';
        
    // 관계: 해당 컨텐츠가 속한 보고서
    report : Association to Report on report.id = $self.report_id;
}