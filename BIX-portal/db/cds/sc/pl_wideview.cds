 ...
 * 매출 장판지 공통 테이블
 */
entity pl_wideview : managed {
    key ver              : String(20);
    key year             : String(6) not null    @title: '회계연도';
    key month            : String(6) not null    @title: '마감월';
    key prj_no           : String(30) not null   @title: '프로젝트 번호';
    key seq              : Integer default 0     @title: 'Platform 구분 pk';
    key src_type         : String(5) default 'S' @title: 'Data 속성(E(매출), WO(WG ORG(본부별)), WA(WG ACCOUNT(고객사별)), P(Platform), S(자회사), D(SFDC(Deal)))';
