...
entity pl_content_info : cuid, managed {
    content_menu_code : String(20) not null  @title: 'PL 메뉴 코드';
    ...
    use_yn            : Boolean default true @title: '사용 여부';
    sub_info          : Composition of many pl_content_info_sub
                            on sub_info.content_info = $self;
};

/**
 * pl 페이지 메뉴구성, 컨텐츠 정보
 */
entity pl_content_info_sub : cuid, managed {
    sub_key      : String(20) not null  @title: 'Key';
    sub_text     : String(20) not null  @title: 'Text';
    sort_order   : Integer not null     @title: '정렬순서';
    use_yn       : Boolean default true @title: '사용 여부';
    content_info : Association to one pl_content_info;
};
