...
view pl_content_view(page_path : String(50), // actual <-> plan (실적 PL <-> 추정 PL)
    ...
    select from (
        select
            row_number() over(order by card.page_path asc, card.position asc, card.grid_layout_info desc, menu.sort_order asc, card.detail_path asc, card.detail_info desc, card.sort_order asc) as row,
            card.page_path,
            card.position,
        ...
        left join common_menu as menu
            on  card.page_path   = menu.page_path
            and card.detail_path = menu.detail_path
            and menu.isApp       = 'sub'
        ...
        where
            ...
            and (
                   card.grid_layout_info = :grid_layout_info
                or ifnull(
                    length( :grid_layout_info), 0
                )                        = 0
            )
            and (
                   card.detail_path      = :detail_path
                or ifnull(
                    length( :detail_path), 0
                )                        = 0
            )
            and (
                   card.detail_info      = :detail_info
                or ifnull(
                    length( :detail_info), 0
                )                        = 0
            )
