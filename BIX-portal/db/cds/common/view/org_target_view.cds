 ...
 *
 * [부문/본부 별]
 * 매출, 마진율, BR(MM), RoHC, Offshore, Non-MM, DT매출, SGA
 */
view org_target_view as
    select from (
        select
            ...
            div_margin_target.is_total_calc        as div_target_margin_amt_is_total_calc,
            ifnull(
                div_margin_target.target_val, 0
            )                                      as div_target_margin_amt        : Decimal(18, 2) @title: '마진액 부문 목표액',
            hdqt_margin_target.is_total_calc       as hdqt_target_margin_amt_is_total_calc,
            ifnull(
                hdqt_margin_target.target_val, 0
            )                                      as hdqt_target_margin_amt       : Decimal(18, 2) @title: '마진액 본부 목표액',

            div_margin_rate_target.is_total_calc   as div_target_margin_rate_is_total_calc,
            ifnull(
                div_margin_rate_target.target_val, 0
            )                                      as div_target_margin_rate       : Decimal(18, 2) @title: '마진율 부문 목표액',
            div_margin_rate_target.is_total_calc   as hdqt_target_margin_rate_is_total_calc,
            ifnull(
                div_margin_rate_target.target_val, 0
            )                                      as hdqt_target_margin_rate      : Decimal(18, 2) @title: '마진율 본부 목표액',
        ...
        left join common_annual_target as div_margin_rate_target
            on(
                    div_margin_rate_target.year        = (
                    select substring(
                        ver, 2, 4
                    ) as year from common_version
                    where
                ...
                and div_margin_rate_target.target_type = 'ccorg_cd'
                and div_margin_rate_target.target_cd   = 'A02'
                and org.div_ccorg_cd                   = div_margin_rate_target.target_type_cd
            )
        left join common_annual_target as hdqt_margin_rate_target
            on(
                    hdqt_margin_rate_target.year        = (
                    select substring(
                        ver, 2, 4
                    ) as year from common_version
                    where
                ...
                and hdqt_margin_rate_target.target_type = 'ccorg_cd'
                and hdqt_margin_rate_target.target_cd   = 'A02'
                and org.hdqt_ccorg_cd                   = hdqt_margin_rate_target.target_type_cd
            )
        left join common_annual_target as div_margin_target
            on(
                ...
                and div_margin_target.target_cd   = 'A03'
        ...
        left join common_annual_target as hdqt_margin_target
            on(
                ...
                and hdqt_margin_target.target_cd   = 'A03'
        ...
        left join common_annual_target as div_br_cost_target
            on(
                ...
                and div_br_cost_target.target_cd   = 'A07'
        ...
        left join common_annual_target as hdqt_br_cost_target
            on(
                ...
                and hdqt_br_cost_target.target_cd   = 'A07'
        ...
        key org_ccorg_cd,
            ...
            div_target_sale_amt,
            div_target_margin_amt_is_total_calc,
            div_target_margin_amt,
            ...
            hdqt_target_margin_amt_is_total_calc,
            hdqt_target_margin_amt,
...
// 본부들의 목표액
view org_target_hdqt as 
    select
    org.*,
    case when target_cd = 'A01' then target_val else 0 end as target_sale
    from common_org_full_level_view as org
    left join common_annual_target as target on org.org_ccorg_cd = target.target_type_cd
    where
        hdqt_ccorg_cd is not null
    and team_ccorg_cd is null;
...
view org_target_sum_view as
        select from (
            select
                ...
                ifnull((
                    select sum(case
                                   when div_target_sale_amt_is_total_calc = true
                                        then ifnull(
                                                 div_target_sale_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                        ...
                        where
                            ...
                            and    org_target.target_year  =      target_year
                            and (
                                   lv1_ccorg_cd            =      org_target.lv1_ccorg_cd
                            )
                            and (
                                   lv2_ccorg_cd            =      org_target.lv2_ccorg_cd
                                or org_target.lv2_ccorg_cd is     null
                            )
                            and (
                                   lv3_ccorg_cd            =      org_target.lv3_ccorg_cd
                                or org_target.lv3_ccorg_cd is     null
                            )
                    )
                ),0)+ifnull((
                    select sum(case
                ...
                ifnull((
                    select sum(case
                                   when div_target_margin_amt_is_total_calc = true
                                        then ifnull(
                                                 div_target_margin_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                        ...
                        from org_target_view
                        where
                                   div_target_margin_amt   is not null
                            and    div_target_margin_amt   <>     0
                            and    org_target.target_year  =      target_year
                            and (
                                   lv1_ccorg_cd            =      org_target.lv1_ccorg_cd
                            )
                    ...
                    select sum(case
                                   when div_target_margin_amt_is_total_calc      <> true
                                        and hdqt_target_margin_amt_is_total_calc =  true
                                        then ifnull(
                                                 hdqt_target_margin_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                        ...
                        from org_target_view
                        where
                                   hdqt_target_margin_amt  is not null
                            and    hdqt_target_margin_amt  <>     0
                            and    org_target.target_year  =      target_year
                            and (
                                   lv1_ccorg_cd            =      org_target.lv1_ccorg_cd
                            )
                ...
                ifnull((
                    select sum(case
                                   when div_target_rohc_is_total_calc = true
                                        then ifnull(
                                                 div_target_rohc, 0
                                             )
                                   else 0
                ...
                ifnull((
                    select sum(case
                                   when div_offshore_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_offshore_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as offshore_target_amt,

                ifnull((
                    select sum(case
                                   when div_non_mm_target_sale_amt_is_total_calc = true
                                        then ifnull(
                                                 div_non_mm_target_sale_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as non_mm_target_sale_amt,


                (
                    select sum(ifnull(
                        div_dt_target_sale_amt, 0
                    )) from (
                        select
                            div_dt_target_sale_amt_is_total_calc,
                            div_dt_target_sale_amt,
                            div_ccorg_cd
                        from org_target_div_view
                        where
                                   div_dt_target_sale_amt               is not null
                            and    div_dt_target_sale_amt               <>     0
                            and    div_dt_target_sale_amt_is_total_calc =      true
                            and    org_target.target_year               =      target_year
                            and (
                                   lv1_ccorg_cd                         =      org_target.lv1_ccorg_cd
                            )
                ...
                ifnull((
                    select sum(case
                                   when div_sga_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_sga_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ifnull((
                    select sum(case
                                   when div_profit_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_profit_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as profit_target_amt,

                ifnull((
                    select sum(case
                                   when div_total_profit_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_total_profit_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                    ...
                    select sum(case
                                   when div_total_profit_target_amt_is_total_calc      <> true
                                        ...
                                        then ifnull(
                                                 hdqt_total_profit_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as total_profit_target_amt,

                ifnull((
                    select sum(case
                                   when div_total_labor_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_total_labor_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as total_labor_target_amt,

                ifnull((
                    select sum(case
                                   when div_labor_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_labor_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as labor_target_amt,

                ifnull((
                    select sum(case
                                   when div_invest_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_invest_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as invest_target_amt,

                ifnull((
                    select sum(case
                                   when div_expense_target_amt_is_total_calc = true
                                        then ifnull(
                                                 div_expense_target_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
                ...
                ),0)   as expense_target_amt,

                ifnull((
                    select sum(case
                                   when div_target_cont_margin_amt_is_total_calc = true
                                        then ifnull(
                                                 div_target_cont_margin_amt, 0
                                             )
                                   else 0
                               end) from (
                        select distinct
        ...
        union all
            ...
            select
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_target_sale_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                             sum(div_target.div_target_margin_amt), 0
                         )                                              =      0
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_target_margin_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when org_target.hdqt_ccorg_cd                       is not null
                         then ifnull(
                                  hdqt_target.hdqt_target_br_mm_amt, 0
                              )
                    else ifnull(
                             div_target.div_target_br_mm_amt, 0
                         )
                ...
                case
                    when org_target.hdqt_ccorg_cd                       is not null
                         then ifnull(
                                  hdqt_target.hdqt_target_br_cost_amt, 0
                              )
                    else ifnull(
                             div_target.div_target_br_cost_amt, 0
                         )
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_target_rohc
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_offshore_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_non_mm_target_sale_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_dt_target_sale_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_sga_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_profit_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_total_profit_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_total_labor_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_labor_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_invest_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_expense_target_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
                ...
                case
                    when ifnull(
                         ...
                         then ifnull(
                                  sum(case
                                          when org_target.hdqt_ccorg_cd is not null
                                               then hdqt_target.hdqt_target_cont_margin_amt
                                          else 0
                                      end), 0
                              )
                    else ifnull(
            ...
            where
                    org_target.div_ccorg_cd is not null
                and org_target.org_id       is not null
            ...
            key target_year             : String(4),
                ...
                target_margin_amt       : Decimal(18, 2),
